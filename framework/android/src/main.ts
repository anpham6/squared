...
const appBase: android.AppFramework<View> = {
    ...
    create() {
        application = new Application<View>(
            ...
            Resource,
            new Map<string, squared.base.ExtensionUI<View>>([
    ...
    addFontProvider(authority: string, packageName: string, certs: string[], webFonts: string | PlainObject) {
        if (application) {
            return (application.resourceHandler as Resource<View>).addFontProvider(authority, packageName, certs, webFonts);
        }
    },
    addDependency(group: string, name: string, version: string) {
        if (application) {
            return application.addDependency(group, name, version, true);
        }
    },
    getLocalSettings() {
