...
this.android.widget.toolbar = (function () {
    ...
    class Toolbar extends squared.base.ExtensionUI {
        ...
        processNode(node, parent) {
            ...
            const collapsingToolbarOptions = createViewAttribute(
                options.collapsingToolbar
            );
            const numberResourceValue = application.extensionManager.optionValueAsBoolean(
                EXT_ANDROID.RESOURCE_STRINGS,
                "numberResourceValue"
            );
            const hasMenu = Toolbar.findNestedElement(
                node,
            ...
            const [controlName, appBarName, collapsingToolbarName] =
                node.api < 29 /* Q */
                    ? [
                          SUPPORT_ANDROID.TOOLBAR,
                          SUPPORT_ANDROID.APPBAR,
                          SUPPORT_ANDROID.COLLAPSING_TOOLBAR,
                      ]
                    : [
                          SUPPORT_ANDROID_X.TOOLBAR,
                          SUPPORT_ANDROID_X.APPBAR,
                          SUPPORT_ANDROID_X.COLLAPSING_TOOLBAR,
                      ];
            const hasCollapsingToolbar =
                "collapsingToolbar" in options ||
                collapsingToolbarChildren.length > 0;
            const hasAppBar =
                "appBar" in options ||
                appBarChildren.length > 0 ||
                hasCollapsingToolbar;
            let appBarOverlay = "";
            let popupOverlay = "";
            ...
            if (hasAppBar) {
                ...
                appBarNode = this.createPlaceholder(
                    node,
                    appBarChildren,
                    node.target
                );
                ...
                if (hasCollapsingToolbar) {
                    ...
                    collapsingToolbarNode = this.createPlaceholder(
                        node,
                        collapsingToolbarChildren
                    );
                    if (collapsingToolbarNode) {
                        ...
                        collapsingToolbarNode.setControlType(
                            collapsingToolbarName,
                            CONTAINER_NODE.BLOCK
                        );
                    }
                }
            }
            ...
            if (appBarNode) {
                ...
                if (collapsingToolbarNode) {
                    ...
                    application.addLayoutTemplate(
                        collapsingToolbarNode.renderParent,
                    ...
                    if (backgroundImage) {
                        ...
                        if (src !== "") {
                            ...
                            const backgroundImageOptions = createViewAttribute(
        ...
        processChild(node) {
            if (
                node.imageElement &&
                (node.dataset.navigationIcon || node.dataset.collapseIcon)
            ) {
                node.hide();
                return { next: true };
            }
        ...
        postOptimize(node) {
            ...
            if (menu) {
                const toolbarOptions = createViewAttribute(
                    (_b = this.options[node.elementId]) === null ||
                        _b === void 0
                        ? void 0
                        : _b.self
                );
            ...
            const themeData = node.data(
                "android.widget.toolbar" /* TOOLBAR */,
                "themeData"
            );
            if (themeData) {
                const options = createStyleAttribute(this.options.resource);
                const optionsActionBar = createStyleAttribute({
                    name: ".NoActionBar",
                    output: options.output,
                });
                const optionsAppBar = createStyleAttribute({
                    name: ".AppBarOverlay",
                    output: options.output,
                });
                const optionsPopup = createStyleAttribute({
                    name: ".PopupOverlay",
                    output: options.output,
                });
                assignEmptyValue(
                ...
                assignEmptyValue(
                    optionsAppBar,
                    "parent",
                    themeData.appBarOverlay ||
                        "ThemeOverlay.AppCompat.Dark.ActionBar"
                );
                assignEmptyValue(
                    optionsPopup,
                    "parent",
                    themeData.popupOverlay || "ThemeOverlay.AppCompat.Light"
                );
