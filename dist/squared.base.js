...
(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined'
        ...
        : ((global = typeof globalThis !== 'undefined' ? globalThis : global || self),
          factory(((global.squared = global.squared || {}), (global.squared.base = {}))));
    ...
    class Application {
        ...
        saveAs(filename, options) {
            ...
            return (
                ((_a = this.fileHandler) === null || _a === void 0
                    ? void 0
                    : _a.saveAs(filename || this.resourceHandler.userSettings.outputArchiveName, options)) ||
                reject(OPERATION_NOT_SUPPORTED)
            );
        }
        ...
        parseDocument(...elements) {
            ...
            const [rootElements, processing, shadowElements, styleSheets] = this.createSessionThread(
                elements,
                this.userSettings.pierceShadowRoot && preloadCustomElements
            );
                            ...
                            if (typeof item === 'string') {
                                fetch(item).then(async result => {
                                    ...
                                    if (
                                        ...
                                        (styleSheets && styleSheets.includes(item))
                                    ) {
                                        ...
                                        success({
                                            mimeType:
                                                ...
                                                'font/' + (splitPair(item, '.', false, true)[1].toLowerCase() || 'ttf'),
            ...
            if (node) {
                ...
                for (let i = 0; i < length; ++i) {
                    ...
                    if (element.nodeName[0] === '#') {
                        ...
                        child = (shadowRoot || element).childNodes.length
            ...
            switch (item.type) {
                case CSSRule.STYLE_RULE: {
                    ...
                    const hasExactValue = (attr, value) =>
                        new RegExp(`\\s*${attr}\\s*:\\s*${value}\\s*;?`).test(cssText);
                    const hasPartialValue = (attr, value) =>
                        new RegExp(`\\s*${attr}\\s*:[^;]*?${value}[^;]*;?`).test(cssText);
                    ...
                    for (let i = 0, length = items.length; i < length; ++i) {
                        ...
                        switch (value) {
                            case 'initial':
                                ...
                                if (
                                    ((_b = CSS_PROPERTIES[baseAttr]) === null || _b === void 0 ? void 0 : _b.value) ===
                                    'auto'
                                ) {
                                    value = 'auto';
                                    break;
                                    ...
                                    required: {
                                        for (const name in CSS_SHORTHANDNONE) {
                                            ...
                                            if (css.value.includes(baseAttr)) {
                                                if (
                                                    hasExactValue(css.name, 'none|initial') ||
                                                    (value === 'initial' && hasPartialValue(css.name, 'initial')) ||
                                                    (css.valueOfNone && hasExactValue(css.name, css.valueOfNone))
                                                ) {
                                                    break required;
                                                }
                                                break;
                                ...
                                if (matchHost) {
                                    if (matchHost[2] === '*') {
                                        ...
                                        const result = document.querySelectorAll(
                                            matchHost[1] === 'host'
                                                ? hostElement.tagName + matchHost[2]
                                                : matchHost[2] + ' ' + hostElement.tagName
                                        );
                ...
                case CSSRule.FONT_FACE_RULE:
                    if (resource) {
                        ...
                        if (attr) {
                            const src =
                                (_e = REGEXP_FONTSRC.exec(attr)) === null || _e === void 0 ? void 0 : _e[1].trim();
                            let fontFamily =
                                (_f = REGEXP_FONTFAMILY.exec(attr)) === null || _f === void 0 ? void 0 : _f[1].trim();
                            if (src && fontFamily) {
                                fontFamily = trimBoth(fontFamily, '"');
                                const fontStyle =
                                    ((_g = REGEXP_FONTSTYLE.exec(attr)) === null || _g === void 0
                                        ? void 0
                                        : _g[1].toLowerCase()) || 'normal';
                                const fontWeight = parseInt(
                                    ((_h = REGEXP_FONTWEIGHT.exec(attr)) === null || _h === void 0 ? void 0 : _h[1]) ||
                                        '400'
                                );
                                for (const value of src.split(',')) {
                                    ...
                                    if (match) {
                                        const data = {
                                            ...
                                            fontStyle,
                                            srcFormat:
                                                ((_j = match[4]) === null || _j === void 0
                                                    ? void 0
                                                    : _j.toLowerCase().trim()) || 'truetype',
                                        };
                    ...
                    for (let i = 0, length = cssRules.length; i < length; ++i) {
                        ...
                        switch (rule.type) {
                            ...
                            case CSSRule.KEYFRAMES_RULE: {
                                ...
                                if (value) {
                                    const keyframesMap =
                                        (_c = this.getProcessing(sessionId)).keyframesMap ||
                                        (_c.keyframesMap = new Map());
                ...
                if (shadowRootItems) {
                    ...
                    for (const shadowRoot of shadowRootItems) {
                        ...
                        shadowRoot.querySelectorAll('link').forEach(element => {
                            ...
                            if (href) {
                                const mimeType =
                                    element.rel.trim() === 'stylesheet'
                                        ? 'text/css'
                                        : element.type.trim() || parseMimeType(href);
    ...
    class ExtensionManager {
        ...
        get(name, builtIn) {
            return (
                this.extensions.find(item => item.name === name) ||
                findSet(this.cache, item => item.name === name) ||
                (builtIn ? this.application.builtInExtensions.get(name) : undefined)
            );
        }
        checkDependencies() {
    ...
    class File {
        ...
        appendFiles(filename, options) {
            return this.archiving(Object.assign(Object.assign({}, options), { appendTo: filename }));
        }
        copyFiles(directory, options) {
    ...
    function convertBorderWidth(node, dimension, border) {
        if (!node.plainText) {
            ...
            if (width) {
                const result = width.endsWith('px')
                    ? parseFloat(width)
                    : isLength(width, true)
                    ? node.parseUnit(width, { dimension })
                    : parseFloat(node.style[border[0]]);
                if (result) {
            ...
            for (let i = 0, length = pseudoList.length; i < length; ++i) {
                ...
                switch (pseudo) {
                    ...
                    case ':placeholder-shown':
                        if (
                            !((tagName === 'INPUT' || tagName === 'TEXTAREA') && child.toElementString('placeholder'))
                        ) {
                            return false;
                        }
                        break;
    ...
    class Node extends squared.lib.base.Container {
            ...
            if (value) {
                ...
                if (ignoreDefault !== true) {
                    ...
                    if (
                        data &&
                        (value === data.value ||
                            (hasBit$1(data.trait, 256 /* UNIT */) && parseFloat(value) === parseFloat(data.value)))
                    ) {
                        return false;
                    }
                }
                        ...
                        if (r && offset !== -1 && offset < depthCount) {
                            ...
                            if (!lastEnd && (dataEnd.adjacent || resultCount !== -Infinity)) {
                                ...
                                for (let j = 0; j < s; ++j) {
                                    ...
                                    if (
                                        (currentCount === 0 || !result.includes(node)) &&
                                        ascendQuerySelector(this, selectors, 0, [node], dataEnd.adjacent)
                                    ) {
                                        result.push(node);
        ...
        get parentElement() {
            var _a;
            return this._element
                ? getParentElement(this._element)
                ...
                if (pageFlow) {
                    if ((this.block && !this.floating) || this.lineBreak) {
                        ...
                        if (actualParent && ((actualParent.block && !actualParent.floating) || actualParent.hasWidth)) {
                            if (
                                this.inlineStatic &&
                                ((_a = this.firstChild) === null || _a === void 0 ? void 0 : _a.blockStatic)
                            ) {
                                result = true;
            ...
            if (result === undefined) {
                if (!this.plainText) {
                    ...
                    if (
                        ...
                        this.styleElement &&
                        this.pageFlow &&
                        ((!this.inputElement && this.opacity === 1) || isTransparent(result))
                    ) {
                ...
                if (this.naturalChild) {
                    if (this.styleElement) {
                        ...
                        if (value.endsWith('px')) {
                            ...
                            result =
                                (value.endsWith('rem')
                                    ? parseFloat(value) * getRemSize(fixedWidth)
                                    : parseUnit(value, { fixedWidth })) * emRatio;
                        }
                    } else {
            ...
            return (
                ...
                (this._style = this.styleElement
                    ? !this.pseudoElt
                        ...
                        : getStyle(getParentElement(this._element), this.pseudoElt)
                    : PROXY_INLINESTYLE)
            );
        }
        get cssStyle() {
    ...
    function appendSeparator(preceding = '', value = '', separator = '/') {
        ...
        return (
            preceding +
            (preceding && value && !preceding.endsWith(separator) && !value.startsWith(separator) ? separator : '') +
            value
        );
    }
    ...
    class Resource {
        ...
        addRawData(uri, mimeType, content, options) {
            ...
            if (encoding === 'base64') {
                ...
                content && (content = content.replace(/\\(["'])/g, (match, ...capture) => capture[0]));
            }
            ...
            if (font) {
                ...
                return font.find(
                    item =>
                        ...
                        (Resource.hasMimeType(mimeType, item.srcFormat) ||
                            (item.srcUrl && Resource.hasMimeType(mimeType, item.srcUrl)))
                );
            }
        }
        getRawData(uri) {
    ...
    const {
        CSS_PROPERTIES: CSS_PROPERTIES$2,
        calculate,
        convertAngle,
        formatPercent,
        formatPX: formatPX$1,
    ...
    const convertLength = (value, dimension, options) =>
        isPercent$1(value) ? Math.round(((parseFloat(value) || 0) / 100) * dimension) : parseUnit$1(value, options);
    const convertPercent = (value, dimension, options) =>
        isPercent$1(value) ? parseFloat(value) / 100 : parseUnit$1(value, options) / dimension;
    const checkPreviousSibling = node =>
        !node || node.lineBreak || node.floating || (node.plainText && CHAR_TRAILINGSPACE.test(node.textContent));
    class ResourceUI extends Resource {
                        ...
                        switch (position) {
                            ...
                            default: {
                                ...
                                if (percent > 1) {
                                    ...
                                    result[position] =
                                        convertLength(formatPercent(percent - 1), offsetParent, {
                                            fontSize,
                                            screenDimension,
                                        }) * -1;
                                } else {
                                    result[direction] = convertLength(position, offsetParent, {
                                        fontSize,
                                        screenDimension,
                                    });
                                }
                        ...
                        if (isLength$1(position, true)) {
                            ...
                            switch (alignment) {
                                ...
                                case 'right': {
                                    ...
                                    const locationAsPercent = convertPercent(position, width, {
                                        fontSize,
                                        screenDimension,
                                    });
                                    if (alignment === 'right') {
                                        ...
                                        if (locationAsPercent > 1) {
                                            ...
                                            result.right = convertLength(formatPercent(percent), width, {
                                                fontSize,
                                                screenDimension,
                                            });
                                ...
                                case 'bottom': {
                                    ...
                                    const locationAsPercent = convertPercent(position, height, {
                                        fontSize,
                                        screenDimension,
                                    });
                                    if (alignment === 'bottom') {
                                        ...
                                        if (locationAsPercent > 1) {
                                            ...
                                            result.bottom = convertLength(formatPercent(percent), height, {
                                                fontSize,
                                                screenDimension,
                                            });
        ...
        static parseBackgroundImage(node, backgroundImage) {
            ...
            const getGradientPosition = value =>
                isString(value) ? (value.includes('at ') ? /(.+?)?\s*at (.+?)\s*$/.exec(value) : [value, value]) : null;
            ...
            while ((match = REGEXP_BACKGROUNDIMAGE.exec(backgroundImage))) {
                ...
                if (value.startsWith('url(') || value === 'initial') {
                    ...
                    const imageDimension = backgroundSize.length
                        ? ResourceUI.getBackgroundSize(node, backgroundSize[i % backgroundSize.length])
                        : null;
                    ...
                    switch (type) {
                        ...
                        case 'radial': {
                            ...
                            const center = ResourceUI.getBackgroundPosition(
                            ...
                            if (match) {
                                if (!nextSibling.textElement) {
                                    nextSibling = nextSibling.find(item => item.naturalChild && item.textElement, {
                                        cascade: true,
                                        error: item => item.naturalChild && !item.textElement && item.isEmpty(),
                                    });
                                }
                                if (nextSibling) {
        ...
        setFontStyle(node) {
            if (
                ((node.textElement || node.inlineText) &&
                    (!node.textEmpty || node.pseudoElement || node.visibleStyle.background)) ||
                (node.inputElement && !node.controlElement)
            ) {
            ...
            if (value) {
                ...
                switch (node.css('whiteSpace')) {
                    ...
                    case 'break-spaces':
                        ...
                        break;
                    case 'pre-line':
                        value = value.replace(/\n/g, ResourceUI.STRING_NEWLINE).replace(/\s{2,}/g, ' ');
                        trimming = false;
                        break;
                    case 'nowrap':
                        inlined = true;
                    default:
                        ...
                        if (node.onlyChild && node.htmlElement) {
                            value = value.replace(CHAR_LEADINGSPACE, '').replace(CHAR_TRAILINGSPACE, '');
                            ...
                            if (!node.horizontalRowStart) {
                                ...
                                if (
                                    ...
                                    !hasEndingSpace(previousSibling) &&
                                    element.textContent.trim().startsWith(value.trim())
                                ) {
                            ...
                            if (
                                node.horizontalRowStart ||
                                ((_a = node.previousSibling) === null || _a === void 0 ? void 0 : _a.blockStatic)
                            ) {
                                value = value.replace(CHAR_LEADINGSPACE, '');
                            }
                    ...
                    if (!node.naturalChild) {
                        ...
                        if (value.length > 1) {
                            if (checkPreviousSibling(previousSibling)) {
                                ...
                                previousSpaceEnd =
                                    hasEndingSpace(previousSibling.element) ||
                                    ((_c = previousSibling.lastStaticChild) === null || _c === void 0
                                        ? void 0
                                        : _c.lineBreak);
                            }
                        }
                        if (inlined) {
                            ...
                            value = value.replace(
                                ...
                                node.display === 'table-cell' ||
                                    ...
                                    node.blockStatic ||
                                    (nextSibling && nextSibling.floating)
                                    ? ''
                                    : ResourceUI.STRING_SPACE
                            );
                        } else if (!node.inlineText) {
                ...
                if (
                    ...
                    if (child) {
                        if (styled && child.htmlElement) {
                            if (child.lineBreak) {
                                value = value.replace(
                                    !preserveWhiteSpace ? new RegExp(`\\s*${item.outerHTML}\\s*`) : item.outerHTML,
                                    child.lineBreakTrailing &&
                                        ((_a = child.previousSibling) === null || _a === void 0
                                            ? void 0
                                            : _a.inlineStatic)
                                        ? ''
                                ...
                                value = value.replace(
                                    item.outerHTML,
                                    child.pageFlow && isString(child.textContent) ? ResourceUI.STRING_SPACE : ''
                                );
                            }
                            return;
                        } else {
    ...
    class NodeUI extends Node {
        ...
        lockedAttr(name, attr) {
            var _a, _b;
            return !!((_b = (_a = this._locked) === null || _a === void 0 ? void 0 : _a[name]) === null || _b === void 0
                ? void 0
                : _b[attr]);
        }
        render(parent) {
        ...
        parseUnit(value, options) {
            ...
            (_a = options || (options = {})).screenDimension ||
                (_a.screenDimension = this.localSettings.screenDimension);
            ...
            switch (module) {
                case 'initial':
                    this._initial = this._initial
                        ? cloneObject(data, { target: this._initial })
                        : Object.assign({}, data);
                    break;
                case 'textStyle':
                    this.cssApply(data);
        ...
        hasResource(value) {
            return this._exclusions === undefined || !hasBit$2(this._exclusions[0], value);
        }
        hasProcedure(value) {
            return this._exclusions === undefined || !hasBit$2(this._exclusions[1], value);
        }
        hasSection(value) {
            return this._exclusions === undefined || !hasBit$2(this._exclusions[2], value);
        }
        exclude(options) {
        ...
        alignedVertically(siblings, cleared, horizontal) {
            if (this.lineBreak) {
                ...
                if (this.autoPosition) {
                    ...
                    for (let i = siblings.length - 1; i >= 0; --i) {
                        ...
                        if (previous.pageFlow) {
                            return previous.blockStatic || (cleared && cleared.has(previous))
                                ? 1 /* VERTICAL */
                                : 0 /* HORIZONTAL */;
                        }
                    }
                ...
                const checkBlockDimension = previous =>
                    this.blockDimension &&
                    Math.ceil(this.bounds.top) >= previous.bounds.bottom &&
                    (this.blockVertical ||
                        previous.blockVertical ||
                        this.percentWidth > 0 ||
                        previous.percentWidth > 0);
                if (isArray(siblings)) {
                    const previous = siblings[siblings.length - 1];
                    const getPercentWidth = node =>
                        node.inlineDimension && !node.hasPX('maxWidth') ? node.percentWidth : -Infinity;
                    if (cleared) {
                        if (
                            cleared.size &&
                            ...
                            if (
                                ...
                                if (siblings.every(item => item.inlineDimension)) {
                                    ...
                                    if (
                                        ...
                                        if (
                                            actualParent.naturalChildren.filter(item => item.visible && item.pageFlow)
                                                .length ===
                                            length + 1
                                        ) {
                                            const getLayoutWidth = node =>
                                                node.actualWidth + Math.max(node.marginLeft, 0) + node.marginRight;
                            ...
                            if (floating && !horizontal && previous.blockStatic) {
                                ...
                                if (
                                    ...
                                    siblings.some(
                                        item =>
                                            Math.floor(top) < item.bounds.top && Math.ceil(bottom) > item.bounds.bottom
                                    )
                                ) {
                                    return 7 /* FLOAT_INTERSECT */;
                ...
                for (let i = length - 1; i >= 0; --i) {
                    ...
                    if (previous.excluded && cleared && cleared.has(previous)) {
                        ...
                        if (
                            this.floatContainer &&
                            this.find(item => item.floating && Math.ceil(item.bounds.top) >= previous.bounds.bottom)
                        ) {
                            return 5 /* FLOAT_BLOCK */;
                        }
                    }
        ...
        modifyBox(region, value, negative = true) {
            ...
            if (value !== 0) {
                ...
                if (node) {
                    ...
                    if (
                        !negative &&
                        (boxReset[index] === 0 ? getSpacingOffset(this, index) : 0) + boxAdjustment[index] + value <= 0
                    ) {
                ...
                if (value !== undefined) {
                    ...
                    if (options.max) {
                        ...
                        if (options.negative === false) {
                            if (
                                (!this._boxReset || this.boxReset[index] === 0 ? getSpacingOffset(this, index) : 0) +
                                    value <=
                                0
                            ) {
                                if (value < 0 && getSpacingOffset(this, index) >= 0) {
            ...
            if (value > 0) {
                if (!this.layoutElement) {
                    ...
                    if (node !== this) {
                        if (node.naturalChild) {
                            if (
                                !node.getBox(attr === 'paddingTop' ? 16 /* PADDING_TOP */ : 64 /* PADDING_BOTTOM */)[0]
                            ) {
                                return 0;
                            }
                        } else {
                    ...
                    if (node.naturalChild) {
                        return canCascadeChildren(node) && cascadeActualPadding(node.naturalElements, attr, value)
                            ? 0
                            : value;
                    }
        ...
        actualTextHeight(options) {
            ...
            const element = createElement$1(tagName !== '#text' ? tagName : 'span', {
                attrs: { textContent: textContent || 'AgjpqyZ' },
                style,
            });
        ...
        getBoxSpacing() {
            ...
            return [
                (!boxReset[0] ? this.marginTop : 0) +
                    this.borderTopWidth +
                    (!boxReset[4] ? this.paddingTop : 0) +
                    boxAdjustment[0] +
                    boxAdjustment[4],
                (!boxReset[1] ? this.marginRight : 0) +
                    this.borderRightWidth +
                    (!boxReset[5] ? this.paddingRight : 0) +
                    boxAdjustment[1] +
                    boxAdjustment[5],
                (!boxReset[2] ? this.marginBottom : 0) +
                    this.borderBottomWidth +
                    (!boxReset[6] ? this.paddingBottom : 0) +
                    boxAdjustment[2] +
                    boxAdjustment[6],
                (!boxReset[3] ? this.marginLeft : 0) +
                    this.borderLeftWidth +
                    (!boxReset[7] ? this.paddingLeft : 0) +
                    boxAdjustment[3] +
                    boxAdjustment[7],
            ];
        }
        ...
        get naturalChild() {
            ...
            if (result === undefined) {
                ...
                return (this._cacheState.naturalChild = !!(
                    element &&
                    (element.parentNode || element === document.documentElement)
                ));
            }
            return result;
        }
        get pseudoElement() {
            ...
            return result === undefined
                ? (this._cacheState.pseudoElement = this._element
                      ? this._element.className === '__squared.pseudo'
                      : false)
                : result;
        }
        get scrollElement() {
        ...
        get documentParent() {
            return (
                ...
                (this._documentParent = this.absoluteParent || this.actualParent || this.parent || this)
            );
        }
            ...
            return result === undefined
                ? (this._cache.inlineFlow =
                      (this.inline ||
                          ...
                          (this.tableElement &&
                              !!((_a = this.previousSibling) === null || _a === void 0 ? void 0 : _a.floating))) &&
                      this.pageFlow)
                : result;
        }
        get blockStatic() {
            ...
            return result === undefined
                ? (this._cache.blockDimension =
                      this.inlineStatic && !this.isEmpty()
                          ? !!((_a = this.firstStaticChild) === null || _a === void 0 ? void 0 : _a.blockStatic)
                          : this.block ||
                            this.inlineDimension ||
                            this.display === 'table' ||
                            this.imageElement ||
        ...
        get verticalAligned() {
            ...
            return result === undefined
                ? (this._cache.verticalAligned =
                      this.verticalAlign !== 0 && !isNaN(parseFloat(this.valueOf('verticalAlign'))))
                : result;
        }
                                    ...
                                    switch (node.size()) {
                                        case 0:
                                            return (
                                                ...
                                                !(node.positionRelative && (node.top !== 0 || node.bottom !== 0))
                                            );
                                        case 1:
        ...
        get onlyChild() {
            ...
            if (!this.documentRoot) {
                const children =
                    ((_a = this.renderParent) === null || _a === void 0 ? void 0 : _a.renderChildren) ||
                    ((_b = this.parent) === null || _b === void 0 ? void 0 : _b.children);
                if (children) {
                    for (let i = 0, length = children.length; i < length; ++i) {
                        const node = children[i];
        ...
        get childIndex() {
            ...
            return result === Infinity && this.innerWrapped
        ...
        get firstLetterStyle() {
            ...
            return result === undefined
                ? (this._cacheState.firstLetterStyle = this.cssPseudoElement('::first-letter') || null)
                : result;
        }
        get firstLineStyle() {
            const result = this._cacheState.firstLineStyle;
            return result === undefined
                ? (this._cacheState.firstLineStyle = this.cssPseudoElement('::first-line') || null)
                : result;
        }
        get textAlignLast() {
    ...
    class LayoutUI extends squared.lib.base.Container {
        ...
        get itemCount() {
            var _a;
            return (_a = this._itemCount) !== null && _a !== void 0 ? _a : this.size();
        }
        get linearX() {
            var _a;
            if (!this._initialized) {
    ...
    function checkPseudoDimension(styleMap, after, absolute) {
        switch (styleMap.display) {
            ...
            case 'unset': {
                ...
                if (
                    (after || !width || (!parseFloat(width) && !isCalc$1(width))) &&
                    (!height || (!parseFloat(height) && !isCalc$1(height)))
                ) {
                    for (const attr in styleMap) {
                        const value = styleMap[attr];
                        if (
                            (/(padding|Width|Height)/.test(attr) && parseFloat(value)) ||
                            (!absolute && attr.startsWith('margin') && parseFloat(value))
                        ) {
                            return true;
                        }
                    }
    ...
    class ApplicationUI extends Application {
        ...
        finalize() {
            ...
            for (let i = 0; i < itemCount; ++i) {
                ...
                if (node.documentRoot && !(!node.rendering && !node.inlineText && node.naturalElements.length)) {
                    ...
                    const renderTemplates =
                        (_a = node.renderParent) === null || _a === void 0 ? void 0 : _a.renderTemplates;
            ...
            for (let i = 0, q = documentRoot.length; i < q; ++i) {
                ...
                this.saveDocument(
                    ...
                    node.dataset['pathname' + capitalize$1(this.systemName)],
                    (
                        (_b = node.renderExtension) === null || _b === void 0
                            ? void 0
                            : _b.some(item => item.documentBase)
                    )
                        ? 0
                        : undefined
                );
        ...
        conditionElement(element, sessionId, cascadeAll, pseudoElt) {
            if (!this._excludedElements.has(element.tagName)) {
                if (this._visibleElement(element, sessionId, pseudoElt) || cascadeAll) {
                    ...
                    const controller = this.controllerHandler;
                    if (
                        iterateArray$2(element.children, item => controller.visibleElement(item, sessionId)) ===
                        Infinity
                    ) {
                        return true;
                    }
                    ...
                    for (let i = 0, length = pseudoElements.length; i < length; ++i) {
                        ...
                        if (item.pageFlow) {
                            ...
                            styleElement = insertStyleSheetRule$1(
                                `${tagName + item.pseudoElt} { display: none !important; }`,
                                0,
                                item.shadowHost
                            );
                        }
        ...
        afterCreateCache(node) {
            ...
            const prefix =
                (isString$1(filename) && filename.replace(this._layoutFileExtension, '')) ||
                node.elementId ||
        ...
        cascadeParentNode(processing, parentElement, sessionId, depth, extensions, shadowParent, cascadeAll) {
            ...
            if (
                ...
                depth === 0 ||
                cascadeAll ||
                node.extensions.some(name => {
                    var _a;
                    return (_a = this.extensionManager.get(name)) === null || _a === void 0 ? void 0 : _a.documentBase;
                })
            ) {
                ...
                for (let i = 0; i < length; ++i) {
                    ...
                    if (element === beforeElement) {
                        ...
                        if (extensions) {
                            const use = this.getDatasetName('use', element);
                            (use ? ApplicationUI.prioritizeExtensions(use, extensions) : extensions).some(item =>
                                item.beforeInsertNode(element, sessionId)
                            );
                        }
                            ...
                            if ((shadowRoot || element).childNodes.length) {
                                ...
                                if (element.tagName === 'svg') {
                                    ...
                                    setElementState(
                                        ...
                                        true,
                                        true,
                                        child.imageElement && FILE$2.SVG.test(child.toElementString('src'))
                                    );
                                }
                            }
                            if (!child.excluded) {
        ...
        cacheNodeChildren(processing, cache, node, children) {
            const length = children.length;
            if (length > 1) {
                let siblingsLeading = [],
                    siblingsTrailing = [],
                            ...
                            if (floated.size && item.pageFlow) {
                                ...
                                if (floated.has(clear) || clear === 'both') {
                                    ...
                                    Math.ceil(item.bounds.top) >=
                                        Math.max(...clearable.map(previous => previous.bounds.bottom))
                                ) {
                    ...
                    for (let i = 0; i < q; ++i) {
                        ...
                        if (
                            ...
                            if (horizontal.length > 1) {
                                const items = horizontal.filter(
                                    item => !item.renderExclude || (floatContainer && clearMap.has(item))
                                );
                                if (items.length > 1) {
                                    const layout = controller.processTraverseHorizontal(
                                ...
                                const items = vertical.filter(
                                    item => !item.renderExclude || (floatContainer && clearMap.has(item))
                                );
                                if (items.length > 1) {
                                    const layout = controller.processTraverseVertical(
                                        new LayoutUI(parentY, nodeY, 0, 0, items),
                                        axisY
                                    );
                        ...
                        if (!nodeY.rendered && nodeY.hasSection(APP_SECTION.EXTENSION)) {
                            ...
                            if (nodeY.styleElement) {
                                combined = nodeY.use
                                    ? ApplicationUI.prioritizeExtensions(nodeY.use, extensionsTraverse)
                                    : extensionsTraverse;
                                for (let j = 0, r = combined.length; j < r; ++j) {
                                    const ext = combined[j];
                                    if (ext.is(nodeY)) {
        ...
        processFloatHorizontal(layout) {
            ...
            for (let i = 0; i < layout.itemCount; ++i) {
                ...
                for (let j = 0; j < itemCount; ++j) {
                    ...
                    const target = controllerHandler.createNodeGroup(seg[0], seg, parent, {
                        delegate: true,
                        cascade: true,
                    });
        ...
        processFloatVertical(layout) {
            ...
            if (layout.containerType !== 0) {
                const wrapper = controllerHandler.createNodeWrapper(layout.node, layout.parent, {
                    containerType,
                    alignmentType,
                    cascade: true,
                });
            ...
            if (styleMap) {
                ...
                if (value) {
                    ...
                    if (!isString$1(textContent)) {
                        if (pseudoElt === '::after') {
                            const checkPseudoAfter = sibling =>
                                sibling.nodeName === '#text' && !/\s+$/.test(sibling.textContent);
                            if (
                                (absolute || !textContent || !checkPseudoAfter(element.lastChild)) &&
                    ...
                    switch (value) {
                        ...
                        default: {
                            ...
                            if (url) {
                                ...
                                const getCounterIncrementValue = (parent, counterName, fallback) => {
                                    var _a;
                                    return getCounterValue(
                                        (_a = getElementCache$2(parent, 'styleMap' + pseudoElt, sessionId)) === null ||
                                            _a === void 0
                                            ? void 0
                                            : _a.counterIncrement,
                                        counterName,
        ...
        setFloatPadding(parent, target, inlineAbove, leftAbove = [], rightAbove = []) {
            const requirePadding = (node, depth) =>
                node.textElement && (node.blockStatic || node.multiline || depth === 1);
            const paddingNodes = [];
                    ...
                    if (marginLeft !== -Infinity) {
                        const offset =
                            ...
                            (parent.box.left +
                                marginLeft +
                                Math.max(
                                    ...target.map(child => (!paddingNodes.includes(child) ? child.marginLeft : 0))
                                ));
                    ...
                    if (marginRight !== -Infinity) {
                        const offset =
                            ...
                            (floatPosition -
                                ...
                                marginRight +
                                Math.max(
                                    ...target.map(child => (!paddingNodes.includes(child) ? child.marginRight : 0))
                                ));
                        if (offset > 0 && offset < parent.actualBoxWidth()) {
                            target.modifyBox(
                                32 /* PADDING_RIGHT */,
    ...
    class ControllerUI extends Controller {
                ...
                switch (element.tagName) {
                    ...
                    case 'BUTTON':
                        ...
                        setButtonStyle(
                            ...
                            this._settingsStyle.inputBackgroundColor
                        );
                        break;
                    case 'TEXTAREA':
                    case 'SELECT':
        ...
        hasAppendProcessing(node) {
            return node
                ? this._beforeOutside.has(node) ||
                      this._beforeInside.has(node) ||
                      this._afterInside.has(node) ||
                      this._afterOutside.has(node)
                : this._requireFormat;
        }
                        ...
                        case 'absolute':
                            if (node.autoPosition) {
                                if (
                                    ...
                                    node.withinX(actualParent.box, { dimension: 'linear' }) &&
                                    node.withinY(actualParent.box, { dimension: 'linear' })
                                ) {
                                        ...
                                        if (!outside) {
                                            ...
                                            if (
                                                ...
                                                (!overflowX &&
                                                    ...
                                                    parent.find(item => item.pageFlow)) ||
                                                (!overflowY &&
            ...
            for (const node of altered) {
                ...
                node.each(item => {
                    if (item.parent !== item.actualParent) {
                        ...
                        if (sibling) {
                            const index =
                                sibling.childIndex + (item.zIndex >= 0 || sibling !== item.actualParent ? 1 : 0);
            ...
            for (let i = 0, length = templates.length; i < length; ++i) {
                ...
                switch (item.type) {
                    case 1 /* XML */: {
                        ...
                        output +=
                            this.getBeforeOutsideTemplate(node, previous) +
                            indent +
                            ...
                            (renderTemplates || beforeInside || afterInside
                                ? '>\n' +
                                  ...
                                  (renderTemplates
                                      ? this.writeDocument(
                                            this.sortRenderPosition(node, renderTemplates),
                                            next,
                                            showAttributes
                                        )
                                      : '') +
        ...
        setElementDimension(element, styleMap, attr, opposing) {
            ...
            if (!dimension || dimension === 'auto') {
                ...
                if (match) {
                    ...
                    !(
                        (element instanceof HTMLObjectElement || element instanceof HTMLEmbedElement) &&
                    ...
                    if (image && image.width && image.height) {
                        ...
                        if (value && isLength$2(value)) {
                            ...
                            if (!styleMap[attrMax] || !attrMax.endsWith('%')) {
                                styleMap[attr] = formatPX$3(
                                    (image[attr] *
                                        parseUnit$2(value, { fontSize: parseFloat(getStyle$2(element).fontSize) })) /
                                        image[opposing]
                                );
                            }
                        }
    ...
    class NodeGroupUI extends NodeUI {
        ...
        get inlineVertical() {
            ...
            return result === undefined
                ? (this._cache.inlineVertical = this.every(node => node.inlineVertical))
                : result;
        }
        get inlineFlow() {
            if (this.hasAlign(32 /* BLOCK */)) {
        ...
        get inlineDimension() {
            ...
            const result = this._cache.inlineDimension;
            return result === undefined
                ? (this._cache.inlineDimension = this.every(node => node.inlineDimension))
                : result;
        }
        get block() {
            if (this.hasAlign(32 /* BLOCK */)) {
        ...
        get blockStatic() {
            ...
            if (result === undefined) {
                ...
                result =
                    (parent.blockStatic && (parent.layoutVertical || this.hasAlign(128 /* COLUMN */))) ||
                    parent.percentWidth > 0 ||
                    (this.layoutVertical &&
                        (parent.hasWidth || !!this.find(node => node.centerAligned || node.rightAligned))) ||
        ...
        get blockDimension() {
            const result = this._cache.blockDimension;
            return result === undefined
                ? (this._cache.blockDimension = this.every(node => node.blockDimension))
                : result;
        }
        get blockVertical() {
            const result = this._cache.blockVertical;
        ...
        get float() {
            const result = this._cache.float;
            return result === undefined
                ? (this._cache.float = !this.floating ? 'none' : this.hasAlign(1024 /* RIGHT */) ? 'right' : 'left')
                : result;
        }
        get floating() {
            const result = this._cache.floating;
            return result === undefined
                ? (this._cache.floating = this.hasAlign(256 /* FLOAT */) || this.every(node => node.floating))
                : result;
        }
        get display() {
            var _a;
            return super.display || ((_a = this.firstChild) === null || _a === void 0 ? void 0 : _a.blockStatic)
                ? 'block'
                : this.blockDimension
                ? 'inline-block'
                : 'inline';
    ...
    class Column extends ExtensionUI {
        ...
        processNode(node, parent) {
            ...
            if (columnGap) {
                ...
                columnGap =
                    (columnWidth && !isNaN(maxSize) && maxSize !== Infinity ? Math.max(maxSize - columnWidth, 0) : 0) +
                    16;
                ...
                columnRule: {
                    ...
                    borderLeftWidth:
                        borderLeftStyle !== 'none'
                            ? borderLeftWidth.endsWith('px')
                                ? parseFloat(borderLeftWidth)
                                : isLength$3(borderLeftWidth, true)
                                ? node.parseUnit(borderLeftWidth)
                                : parseFloat(node.style.borderLeftWidth)
                            : 0,
                    borderLeftColor,
                },
    ...
    const convertLength$1 = (node, value, index) =>
    ...
    class CssGrid extends ExtensionUI {
        ...
        static isJustified(node) {
            return (
                ...
                /^space-|center|flex-end|end|right/.test(node.valueOf('justifyContent'))
            );
        }
        ...
        processNode(node) {
            ...
            const layout = [];
            const gridTemplates = [
                node.valueOf('gridTemplateRows'),
                node.valueOf('gridTemplateColumns'),
                        ...
                        switch (index) {
                            ...
                            case 1:
                                if (command[0] === '[') {
                                    ...
                                    if (iterations) {
                                        ...
                                        while ((subMatch = REGEXP_REPEAT.exec(match[3]))) {
                                            ...
                                            if ((namedMatch = REGEXP_CELL_NAMED.exec(subPattern))) {
                                                ...
                                                repeating.push({
                                                    unit: convertLength$1(node, namedMatch[2], index),
                                                    unitMin: convertLength$1(node, namedMatch[1], index),
                                                });
                                            } else if ((namedMatch = REGEXP_CELL_FIT_CONTENT.exec(subPattern))) {
                                                repeating.push({
                                                    unit: convertLength$1(node, namedMatch[1], index),
                                                    unitMin: '0px',
                                                });
                                            } else if ((namedMatch = REGEXP_CELL_UNIT.exec(subPattern))) {
                                                repeating.push({ unit: convertLength$1(node, namedMatch[0], index) });
                                            }
                                        }
                                    ...
                                    unit.push(convertLength$1(node, match[6], index));
                                    unitMin.push(convertLength$1(node, match[5], index));
                                    repeat.push(false);
                                    ++i;
                                } else if (command.startsWith('fit-content')) {
                                    unit.push(convertLength$1(node, match[7], index));
                                    unitMin.push('0px');
                                    repeat.push(false);
                                    ++i;
                                } else if (REGEXP_UNIT.test(command)) {
                                    unit.push(convertLength$1(node, command, index));
                                    unitMin.push('');
                                    repeat.push(false);
                                    ++i;
                                }
            ...
            if (
                ...
                node.each((item, index) => {
                    if (
                        ...
                        (columnIndex && columnIndex === columnMax)
                    ) {
            ...
            node.each((item, index) => {
                ...
                while (!placement[0] || !placement[1]) {
                    ...
                    if (!PLACEMENT[rowA]) {
                        for (
                            let i = dense ? 0 : getOpenRowIndex(openCells), j = 0, k = -1, length = rowData.length;
                            i < length;
                            ++i
                        ) {
                                            ...
                                            for (let j = outside[0]; j < outside[1]; ++j) {
                                                for (let k = 1; k < length; ++k) {
                                                    ...
                                                    for (let n = 0, r = avail.length; n < r; ++n) {
                                                        ...
                                                        if (
                                                            j >= insideA &&
                                                            (insideB === -1 || j + COLUMN_SPAN <= insideB)
                                                        ) {
                                                            PLACEMENT[colA] = j + 1;
                                                            break found;
                                                        }
    ...
    class Grid extends ExtensionUI {
                ...
                if (node.display === 'table') {
                    ...
                    for (let i = 0; i < size; ++i) {
                        ...
                        if (
                            ...
                            item.pageFlow &&
                            !item.find(child => !checkAlignment(child) || child.percentWidth > 0)
                        ) {
                            if (item.size() > 1) {
                                minLength = true;
                            }
                    ...
                    return (
                        itemCount === size ||
                        (minLength && node.every(item => !item.isEmpty() && NodeUI.linearData(item.children).linearX))
                    );
                }
            }
            return false;
        ...
        processNode(node) {
            ...
            const mapValues = Array.from(nextMapX)
                .sort((a, b) => a[0] - b[0])
                .map(item => item[1]);
            let columnLength = -1;
            for (let i = 0; i < length; ++i) {
                            ...
                            if (columnEnd.length) {
                                ...
                                for (let k = 0, q = children.length; k < q; ++k) {
                                    ...
                                    if (
                                        ...
                                        !sibling.excluded &&
                                        sibling.withinX(
                                            { left: item.linear.right, right: columnEnd[index] },
                                            { dimension: 'linear' }
                                        )
                                    ) {
        ...
        is(node) {
            return (
                ...
                !!node.find(item => {
                    ...
                    return (
                        ((type || ((_a = item.innerBefore) === null || _a === void 0 ? void 0 : _a.pageFlow)) &&
                            isListItem(item)) ||
                        (!type && item.marginLeft < 0 && hasSingleImage(item.visibleStyle))
                    );
                })
            );
        }
            ...
            if (renderParent.support.positionTranslation) {
                ...
                if (
                    ...
                    if (
                        (top < 0 || bottom > 0) &&
                        node.floating &&
                        node.bounds.height > node.documentParent.box.height
                    ) {
    ...
    class Sprite extends ExtensionUI {
        ...
        condition(node) {
            ...
            if (image) {
                ...
                const [backgroundPositionX, backgroundPositionY, backgroundSize] = node.cssAsTuple(
                    'backgroundPositionX',
                    'backgroundPositionY',
                    'backgroundSize'
                );
                const position = ResourceUI.getBackgroundPosition(
    ...
    const { isNumber: isNumber$4, replaceMap } = squared.lib.util;
    var LAYOUT_TABLE;
    (function (LAYOUT_TABLE) {
        LAYOUT_TABLE[(LAYOUT_TABLE['NONE'] = 0)] = 'NONE';
        LAYOUT_TABLE[(LAYOUT_TABLE['STRETCH'] = 1)] = 'STRETCH';
        LAYOUT_TABLE[(LAYOUT_TABLE['FIXED'] = 2)] = 'FIXED';
        LAYOUT_TABLE[(LAYOUT_TABLE['VARIABLE'] = 3)] = 'VARIABLE';
        LAYOUT_TABLE[(LAYOUT_TABLE['COMPRESS'] = 4)] = 'COMPRESS';
        ...
        processNode(node) {
            ...
            const [horizontal, vertical] = !borderCollapse
                ? replaceMap(node.css('borderSpacing').split(' '), (value, index) =>
                      index === 0 ? node.parseUnit(value) : node.parseHeight(value)
                  )
                : [0, 0];
                        ...
                        if (!columnWidth || columnWidth === 'auto') {
                            ...
                            if (
                                ...
                                (width === mapBounds[j] &&
                                    ((percent && length) ||
                                        (percent &&
                                            isPercent$3(mapWidth[j]) &&
                                            td.parseUnit(columnWidth) >= td.parseUnit(mapWidth[j])) ||
                                        (length &&
                                            isLength$6(columnWidth) &&
                                            td.parseUnit(columnWidth) > td.parseUnit(mapWidth[j]))))
                            ) {
                                mapWidth[j] = columnWidth;
                            }
                ...
                if (mapWidth.reduce((a, b) => a + parseFloat(b), 0) > 1) {
                    ...
                    replaceMap(mapWidth, value => {
                        ...
                        if (percentTotal <= 0) {
                            ...
                            value = formatPercent$2(percentTotal / 100);
                        }
            ...
            mainData.layoutType = (() => {
                if (mapWidth.length > 1) {
                    mapPercent = mapWidth.reduce((a, b) => a + (isPercent$3(b) ? parseFloat(b) : 0), 0);
                    if (
                        mainData.layoutFixed &&
                        mapWidth.reduce((a, b) => a + (isLength$6(b) ? parseFloat(b) : 0), 0) >= node.actualWidth
                    ) {
                        return 4 /* COMPRESS */;
                    } else if (
                        (mapWidth.length > 1 && mapWidth.some(value => isPercent$3(value))) ||
                        mapWidth.every(value => isLength$6(value) && value !== '0px')
                    ) {
                        return 3 /* VARIABLE */;
                        ...
                        if (node.find(td => td.hasHeight, { cascade: true })) {
                            ...
                            return hasWidth
                                ? 3 /* VARIABLE */
                                : table.some(tr => tr.find(td => td.multiline))
                                ? 3 /* VARIABLE */
                                : 0 /* NONE */;
                        } else if (hasWidth) {
                            return 2 /* FIXED */;
                    ...
                    if (columnWidth) {
                        switch (mainData.layoutType) {
                            ...
                            case 3 /* VARIABLE */:
                                ...
                                } else if (isLength$6(columnWidth) && parseInt(columnWidth)) {
            ...
            if (mainData.borderCollapse) {
                ...
                let hideTop, hideRight, hideBottom, hideLeft;
                for (let i = 0; i < rowCount; ++i) {
                    const tr = tableFilled[i];
                    for (let j = 0; j < columnCount; ++j) {
                        const td = tr[j];
    ...
    const canResetChild = (node, children = true) =>
        ((!children && node.blockStatic) || (children && !node.isEmpty() && !node.floating)) &&
        !node.layoutElement &&
        !node.tableElement &&
    ...
    const hasOverflow = node =>
        checkOverflowValue(node.valueOf('overflowY')) || checkOverflowValue(node.valueOf('overflowX'));
    const isLowestElement = (node, siblings) =>
        node.linear.bottom >= minMaxOf(siblings, sibling => sibling.linear.bottom, '>')[1];
            ...
            cache.each(node => {
                ...
                if (node.naturalElement && !node.hasAlign(2 /* AUTO_LAYOUT */)) {
                    ...
                    const collapseMargin =
                        ...
                        ((_a = node.actualParent) === null || _a === void 0 ? void 0 : _a.layoutElement) !== true &&
                        hasOverflow(node) &&
                        node.tagName !== 'FIELDSET';
                                            ...
                                            if (node.layoutVertical) {
                                                ...
                                                if (
                                                    previousSiblings.some(
                                                        sibling =>
                                                            sibling.float === 'right' && currentTop > sibling.bounds.top
                                                    )
                                                ) {
                                                    const previousTop = previous.bounds.top;
                                                    const aboveFloating =
                                                        previousSiblings.length > 1
                                                            ? previousSiblings.filter(
                                                                  sibling =>
                                                                      sibling.floating &&
                                                                      previousTop === sibling.bounds.top
                                                              )
                                                            : previousSiblings;
                                                    const [nearest, previousBottom] = minMaxOf(
                                                        aboveFloating,
                                                        sibling => sibling.linear.bottom,
                                                        '>'
                                                    );
                                        ...
                                        if (isBlockElement(previous, false)) {
                                            ...
                                            while (validAboveChild(inherit, true)) {
                                                ...
                                                if (bottomChild && !bottomChild.getBox(4 /* MARGIN_BOTTOM */)[0]) {
                                                    ...
                                                    while (
                                                        ...
                                                        if (
                                                            currentChild.every(item => item.floating || !item.pageFlow)
                                                        ) {
                                                    ...
                                                    if (childBottom !== 0) {
                                                        resetBox(
                                                            bottomChild,
                                                            4 /* MARGIN_BOTTOM */,
                                                            !previous.getBox(4 /* MARGIN_BOTTOM */)[0]
                                                                ? previous
                                                                : undefined
                                                        );
                                                    }
                                            ...
                                            while (validBelowChild(inherit, true)) {
                                                ...
                                                if (
                                                    ...
                                                    !topChild.getBox(1 /* MARGIN_TOP */)[0]
                                                ) {
                                                    ...
                                                    if (childTop !== 0) {
                                                        resetBox(
                                                            topChild,
                                                            1 /* MARGIN_TOP */,
                                                            !current.getBox(1 /* MARGIN_TOP */)[0] ? current : undefined
                                                        );
                                                    }
                                            ...
                                            if (
                                                ...
                                                if (previous.bounds.height === 0) {
                                                    ...
                                                    if (direction) {
                                                        switch (
                                                            (_b = previous.elementData['styleMap::after']) === null ||
                                                            _b === void 0
                                                                ? void 0
                                                                : _b.clear
                                                        ) {
                    ...
                    if (lastChild && (!belowFloating || isLowestElement(lastChild, belowFloating))) {
                        ...
                        if (
                            bottomChild.marginBottom >= node.marginBottom &&
                            !bottomChild.getBox(4 /* MARGIN_BOTTOM */)[0]
                        ) {
                            (node.registerBox(4 /* MARGIN_BOTTOM */) || node).setCacheValue(
                                'marginBottom',
                ...
                if (node.lineBreak && !node.lineBreakTrailing && !clearMap.has(node) && !processed.has(node)) {
                    ...
                    if (q) {
                        ...
                        if (r) {
                            ...
                            if (above.rendered && below.rendered) {
                                ...
                                if (above.textElement && !above.multiline) {
                                    ...
                                    if (above.has('lineHeight')) {
                                        ...
                                        value = above.layoutVertical
                                            ? (_a = above.lastStaticChild) === null || _a === void 0
                                                ? void 0
                                                : _a.lineHeight
                                            : Math.max(...above.map(item => item.lineHeight));
                                    }
                                    if (value) {
                                ...
                                if (offset >= 1) {
                                    ...
                                    if (top) {
                                        ...
                                        const bottom = !above.visible
                                            ? above.registerBox(4 /* MARGIN_BOTTOM */)
                                            : above;
                                        if (bottom) {
                            ...
                            const rect =
                                previousStart.bounds.height === 0 && !previousStart.isEmpty()
                                    ? previousStart.outerRegion
                                ...
                                if (previousStart.rendered || actualParent.visibleStyle.background) {
                                    ...
                                    actualParent.css(
                                        ...
                                        formatPX$6(
                                            Math.max(
                                                offset,
                                                actualParent.hasPX('minHeight', { percent: false })
                                                    ? actualParent.parseHeight(actualParent.css('minHeight'))
                                                    : 0
                                            )
                                        )
                                    );
                    ...
                    if (renderParent && !renderParent.hasAlign(2 /* AUTO_LAYOUT */)) {
                        if (node.blockDimension && !node.floating) {
                            if (renderParent.layoutVertical) {
                                ...
                                for (let i = 0, length = renderChildren.length; i < length; ++i) {
                                    if (renderChildren[i] === outerWrapper) {
                                        if (i > 0 && !node.lineBreakLeading && !node.baselineAltered) {
                                            ...
                                            if (
                                                previous.pageFlow &&
                                                (!previous.blockStatic || (node.inlineStatic && node.blockDimension))
                                            ) {
                                                setSpacingOffset(
                                                    outerWrapper,
                                                    1 /* MARGIN_TOP */,
                                ...
                                const validSibling = item =>
                                    item.pageFlow && item.blockDimension && !item.floating && !item.excluded;
                                let horizontal;
                                            ...
                                            for (let j = 0; j < q; ++j) {
                                                if (outerWrapper === row[j]) {
                                                    if (i > 0) {
                                                        ...
                                                        if (
                                                            !isBlockElement(previousRow[r - 1], false) ||
                                                            !isBlockElement(outerWrapper, true)
                                                        ) {
                                                            let maxBottom = -Infinity;
                                                            for (let k = 0; k < r; ++k) {
                                                                ...
                                                                if (validSibling(innerWrapped)) {
                                                                    maxBottom = Math.max(
                                                                        innerWrapped.actualRect('bottom'),
                                                                        maxBottom
                                                                    );
                                                                }
                                                            }
                        ...
                        if (
                            actualParent.inlineStatic &&
                            actualParent.marginLeft > 0 &&
                            actualParent.firstStaticChild === node &&
                            renderParent.renderParent.outerMostWrapper.layoutVertical
                        ) {
