...
(function (global, factory) {
    ...
    let REGEX_IMPORTANT;
    let REGEX_FONT_FACE;
    let REGEX_FONT_FAMILY;
    let REGEX_FONT_SRC;
    let REGEX_FONT_STYLE;
    let REGEX_FONT_WEIGHT;
    let REGEX_URL;
    ...
    class Application {
        ...
        parseDocument(...elements) {
            ...
            for (const [uri, data] of ASSETS.rawData.entries()) {
                ...
                if (((_a = mimeType) === null || _a === void 0 ? void 0 : _a.startsWith('image/')) && !mimeType.endsWith('svg+xml')) {
                    const element = document.createElement('img');
        ...
        createNode(element, append = true, parent, children) {
            const node = new this._nodeConstructor(this.nextId, this.processing.sessionId, element, this._nodeAfterInsert);
            if (parent) {
        ...
        applyStyleRule(item) {
            ...
            switch (item.type) {
                ...
                case CSSRule.FONT_FACE_RULE: {
                    ...
                    const attr = (_b = REGEX_FONT_FACE.exec(cssText)) === null || _b === void 0 ? void 0 : _b[1];
                    if (attr) {
                        const fontFamily = (((_c = REGEX_FONT_FAMILY.exec(attr)) === null || _c === void 0 ? void 0 : _c[1]) || '').trim();
                        const srcMatch = (((_d = REGEX_FONT_SRC.exec(attr)) === null || _d === void 0 ? void 0 : _d[1]) || '').split(XML.SEPARATOR);
                        if (fontFamily !== '' && srcMatch.length) {
                            const fontStyle = ((_e = REGEX_FONT_STYLE.exec(attr)) === null || _e === void 0 ? void 0 : _e[1].toLowerCase()) || 'normal';
                            const fontWeight = parseInt(((_f = REGEX_FONT_WEIGHT.exec(attr)) === null || _f === void 0 ? void 0 : _f[1]) || '400');
    ...
    const { aboveRange, belowRange, convertCamelCase: convertCamelCase$1, convertFloat, convertInt, filterArray, hasBit: hasBit$1, hasValue, isNumber, isObject: isObject$1, isString: isString$1, spliceArray, spliceString } = $lib$2.util;
    ...
    function setNaturalChildren(node) {
        ...
        if (node.naturalElement) {
            ...
            let i = 0;
            children = [];
            node.element.childNodes.forEach((child) => {
                const childNode = getElementAsNode(child, sessionId);
                if (childNode) {
    ...
    class Node extends squared.lib.base.Container {
        ...
        static getPseudoElt(node) {
            return getElementCache$1(node.element, 'pseudoElement', node.sessionId) || '';
        }
        init() {
                    ...
                    if (!this.pseudoElement) {
                        ...
                        if (items.length) {
                            ...
                            for (const attr of items) {
                                ...
                                const value = checkStyleValue$1(element, name, inline.getPropertyValue(attr), style);
                                if (value !== '') {
                                    styleMap[name] = value;
                                }
                            }
                        ...
                        this.style = getStyle$1(element.parentElement, Node.getPseudoElt(this));
                    }
                }
                else {
                    this.style = getStyle$1(element);
                }
        ...
        saveAsInitial(overwrite = false) {
            ...
            if (bounds) {
                initial.bounds = Object.assign({}, bounds);
            }
            initial.iteration++;
        }
        ...
        cssSpecificity(attr) {
            ...
            if (this.styleElement) {
                const element = this._element;
                return ((_a = getElementCache$1(this.pseudoElement ? element.parentElement : element, 'styleSpecificity' + Node.getPseudoElt(this), this.sessionId)) === null || _a === void 0 ? void 0 : _a[attr]) || 0;
            }
            return 0;
        }
        cssTry(attr, value) {
        ...
        get backgroundImage() {
            ...
            if (result === undefined) {
                ...
                if (value !== '' && value !== 'none' && value !== 'initial') {
                    ...
                    result = ((_a = REGEX_BACKGROUND.exec(this.css('background'))) === null || _a === void 0 ? void 0 : _a[1]) || '';
                }
        ...
        get visibleStyle() {
            ...
            if (result === undefined) {
                if (this.plainText) {
                    ...
                    if (backgroundImage) {
                        for (const repeat of this.css('backgroundRepeat').split(XML$1.SEPARATOR)) {
        ...
        get actualWidth() {
            ...
            if (result === undefined) {
                if (this.plainText) {
                    ...
                    if (((_a = parent) === null || _a === void 0 ? void 0 : _a.flexElement) && parent.css('flexDirection').startsWith('row') || this.display === 'table-cell') {
        ...
        get actualHeight() {
            ...
            if (result === undefined) {
                if (!this.plainText) {
                    const parent = this.actualParent;
                    if (((_a = parent) === null || _a === void 0 ? void 0 : _a.flexElement) && parent.css('flexDirection').startsWith('column') || this.display === 'table-cell') {
        ...
        get fontSize() {
            ...
            if (result === undefined) {
                ...
                if (result === 0 && !this.naturalChild) {
                    ...
                    result = element ? getFontSize(getStyle$1(element)) : NaN;
                }
                while (isNaN(result)) {
                    const parent = this.actualParent;
                    if (parent) {
    ...
    class NodeUI extends Node {
        ...
        static baseline(list, text = false) {
            list = filterArray$1(list, item => {
                if ((item.baseline || isLength$1(item.verticalAlign)) && (!text || item.textElement) && !item.floating && !item.baselineAltered) {
        ...
        setExclusions() {
            var _a;
            if (this.styleElement) {
                const dataset = ((_a = this.actualParent) === null || _a === void 0 ? void 0 : _a.dataset) || {};
        ...
        previousSiblings(options = {}) {
            ...
            const result = [];
            let element = (((_a = this.element) === null || _a === void 0 ? void 0 : _a.previousSibling) || ((_c = (_b = this.firstChild) === null || _b === void 0 ? void 0 : _b.element) === null || _c === void 0 ? void 0 : _c.previousSibling));
            while (element) {
        ...
        nextSiblings(options = {}) {
            ...
            const { floating, pageFlow, lineBreak, excluded } = options;
            const result = [];
            let element = (((_a = this.element) === null || _a === void 0 ? void 0 : _a.nextSibling) || ((_c = (_b = this.lastChild) === null || _b === void 0 ? void 0 : _b.element) === null || _c === void 0 ? void 0 : _c.nextSibling));
            while (element) {
                const node = getElementAsNode$1(element, this.sessionId);
                if (node) {
                    if (lineBreak !== false && node.lineBreak || excluded !== false && node.excluded && !node.lineBreak) {
        ...
        modifyBox(region, offset, negative = true) {
            ...
            if (offset !== 0) {
                ...
                if (attr) {
                    ...
                    const nodes = (_a = this._boxRegister) === null || _a === void 0 ? void 0 : _a[region];
                    if (nodes) {
                        for (const node of nodes) {
        ...
        get onlyChild() {
            ...
            if (!this.documentRoot) {
                return (_b = (_a = this.renderParent) === null || _a === void 0 ? void 0 : _a.renderChildren.length, (_b !== null && _b !== void 0 ? _b : (_c = this.parent) === null || _c === void 0 ? void 0 : _c.length)) === 1;
            }
            return false;
        }
        get textEmpty() {
    ...
    const { aboveRange: aboveRange$3, captureMap, convertFloat: convertFloat$1, convertInt: convertInt$1, convertWord, filterArray: filterArray$2, flatArray, fromLastIndexOf: fromLastIndexOf$2, hasBit: hasBit$4, isString: isString$2, partitionArray, trimString: trimString$1 } = $lib$4.util;
    ...
    function prioritizeExtensions(value, extensions) {
        if (value) {
            const included = value.split(XML$2.SEPARATOR);
            const result = [];
            const untagged = [];
    ...
    class ApplicationUI extends Application {
        ...
        createNode(element, append = true, parent, children) {
            const processing = this.processing;
            const node = new this._nodeConstructor(this.nextId, processing.sessionId, element, this.controllerHandler.afterInsertNode);
            if (parent) {
                node.depth = parent.depth + 1;
        ...
        afterCreateCache(element) {
            ...
            const suffix = (iteration ? convertInt$1(iteration) : -1) + 1;
                ...
                for (let i = 0; i < length; i++) {
                    ...
                    if (floatedRows[i] === null && pageFlow.length) {
                        ...
                        if (pageFlow.length || floating.length) {
                            ...
                            const group = new LayoutUI(node, basegroup, verticalMargin.containerType, verticalMargin.alignmentType);
                            const children = [];
                            let subgroup;
                            if (floating.length) {
    ...
    class ControllerUI extends Controller {
        ...
        applyDefaultStyles(element) {
            ...
            if (isTextNode$3(element)) {
                ...
                const checkBorderAttribute = (index) => !(styleMap[BOX_BORDER$1[0][index]] || styleMap[BOX_BORDER$1[1][index]] || styleMap[BOX_BORDER$1[2][index]] || styleMap[BOX_BORDER$1[3][index]]);
    ...
    class NodeGroupUI extends NodeUI {
        ...
        previousSiblings(options) {
            ...
            return ((_a = node) === null || _a === void 0 ? void 0 : _a.previousSiblings(options)) || [];
        }
        nextSiblings(options) {
            var _a;
            const children = this._initial.children || this.children;
            const node = children[children.length - 1];
            return ((_a = node) === null || _a === void 0 ? void 0 : _a.nextSiblings(options)) || [];
        }
        get block() {
            let result = this._cached.block;
            if (result === undefined) {
    ...
    const { convertCamelCase: convertCamelCase$2, convertFloat: convertFloat$3, hasValue: hasValue$1, isEqual: isEqualObject, isNumber: isNumber$1, isString: isString$3, trimEnd, trimStart } = $lib$7.util;
    const STRING_SPACE = '&#160;';
        ...
        switch (node.css('whiteSpace')) {
            ...
            default:
                if (node.onlyChild && node.htmlElement) {
                    value = value
                        .replace(CHAR$1.LEADINGSPACE, '')
                        .replace(CHAR$1.TRAILINGSPACE, '');
                }
                else {
    ...
    const getGradientPosition = (value) => isString$3(value) ? (value.indexOf('at ') !== -1 ? /(.+?)?\s*at (.+?)\s*$/.exec(value) : [value, value]) : null;
    class ResourceUI extends Resource {
                        ...
                        switch (type) {
                            ...
                            case 'radial': {
                                ...
                                if (radius === 0 && radiusExtent === 0) {
                                    ...
                                    const extent = ((_c = (_b = position) === null || _b === void 0 ? void 0 : _b[1]) === null || _c === void 0 ? void 0 : _c.split(' ').pop()) || '';
                                    switch (extent) {
                                        case 'closest-corner':
                                        case 'closest-side':
                                        case 'farthest-side':
        ...
        static isInheritedStyle(node, attr) {
            var _a;
            return node.styleElement && node.cssInitial(attr) === '' && node.style[attr] === ((_a = node.actualParent) === null || _a === void 0 ? void 0 : _a.style[attr]);
        }
                        ...
                        if (trimming) {
                            ...
                            if (inlined) {
                                ...
                                if (trailingSpace) {
                                    ...
                                    if (((_b = nextSibling) === null || _b === void 0 ? void 0 : _b.blockStatic) === false) {
                                        value += STRING_SPACE;
                                    }
                                }
                            }
    ...
    const { assignRect: assignRect$1 } = $lib$8.dom;
                    ...
                    if (node.cssTry('display', 'block')) {
                        node.each((item) => {
                            ...
                            if (bounds) {
                                ...
                                item.initial.bounds = assignRect$1(rect);
                            }
                        });
    ...
    const { assignRect: assignRect$2 } = squared.lib.dom;
                        ...
                        if (item.cssTry('align-self', 'start')) {
                            if (item.cssTry('justify-self', 'start')) {
                                ...
                                if (bounds) {
                                    ...
                                    item.initial.bounds = assignRect$2(rect);
                                }
                            ...
                            if (columnEnd.length) {
                                ...
                                for (const sibling of item.documentParent.naturalChildren) {
                                    if (!assigned.has(sibling) && sibling.visible && !sibling.rendered) {
                                        ...
                                        if (aboveRange$4(linear.left, item.linear.right) && belowRange$2(linear.right, columnEnd[l])) {
    ...
    const { assignRect: assignRect$3 } = $lib$a.dom;
                    ...
                    if (item === node) {
                        if (preceding) {
                            if (renderParent.layoutVertical && (node.top !== 0 || node.bottom !== 0)) {
                                const bounds = assignRect$3(node.boundingClientRect);
                                if (top !== 0) {
    ...
    const REGEX_BACKGROUND$1 = /rgba\(0, 0, 0, 0\)|transparent/;
        ...
        processNode(node) {
                ...
                tr.each((td, j) => {
                    ...
                    if (!td.visibleStyle.backgroundImage && !td.visibleStyle.backgroundColor) {
                        if (colgroup) {
                            ...
                            if (backgroundColor && !REGEX_BACKGROUND$1.test(backgroundColor)) {
                            ...
                            value = getInheritedStyle$2(element, 'backgroundColor', REGEX_BACKGROUND$1, 'TABLE');
                            if (value !== '') {
                    ...
                    switch (td.tagName) {
                        case 'TH': {
                            function setBorderStyle(attr) {
                                ...
                                td.ascend({ parent: node }).some((item) => {
            ...
            for (let i = 0; i < rowCount; i++) {
                ...
                for (const td of tr.duplicate()) {
                    const { rowSpan, colSpan } = td.element;
                ...
                if (lastChild && columnIndex[i] < columnCount) {
                    const data = lastChild.data(TABLE, 'cellData');
                    if (data) {
    ...
    function resetMargin(node, value) {
        ...
        if (node.getBox(value)[0] === 0) {
            ...
            for (const outerWrapper of node.ascend({ attr: 'outerWrapper' })) {
                    ...
                    if (previousSiblings.length) {
                        ...
                        if (nextSiblings.length) {
                            ...
                            if (inline) {
                                ...
                                (_b = aboveLineBreak) === null || _b === void 0 ? void 0 : _b.setBounds(false);
                            }
