...
(function (global, factory) {
    ...
    class ExtensionManager {
        ...
        include(ext) {
            ...
            if (index !== -1) {
                    ...
                    ext.dependencies.forEach((item) => {
                        if (item.preload) {
                            ...
                            if (this.retrieve(name) === null) {
                                const extension =
                                    application.builtInExtensions[name];
                                if (extension) {
                ...
                if (
                    (framework === 0 ||
                        hasBit(framework, application.framework)) &&
                    ext.dependencies.every((item) => !!this.retrieve(item.name))
                ) {
        ...
        exclude(ext) {
            ...
            for (let i = 0; i < length; ++i) {
                if (
                    extensions[i] === ext ||
                    (typeof ext === "string" && this.retrieve(ext))
                ) {
        ...
        retrieve(name, checkBuiltIn = false) {
            const extensions = this.extensions;
            const length = extensions.length;
            let i = 0;
            while (i < length) {
            ...
            return (
                (checkBuiltIn && this.application.builtInExtensions[name]) ||
                null
            );
        }
        optionValue(name, attr) {
            var _a;
            const options =
                (_a = this.retrieve(name)) === null || _a === void 0
                    ? void 0
                    : _a.options;
    ...
    class NodeList extends squared.lib.base.Container {
        ...
        append(node, delegate = false, cascade = false) {
            ...
            super.append(node);
            if (delegate) {
                (_a = this.afterAppend) === null || _a === void 0
                    ? void 0
                    : _a.call(this, node, cascade);
            }
            return this;
        }
        reset() {
    ...
    class Resource {
        ...
        addRawData(uri, mimeType, encoding, content, width = 0, height = 0) {
            ...
            if (imageMimeType === "*" || imageMimeType.includes(mimeType)) {
                ...
                const filename = uri.endsWith(ext)
                    ? fromLastIndexOf(uri, "/", "\\")
                    : this.randomUUID + ext;
        ...
        getFont(fontFamily, fontStyle = "normal", fontWeight) {
            const font = Resource.ASSETS.fonts.get(
            ...
            if (font) {
                ...
                return font.find(
                    (item) =>
                        ...
                        (!fontWeight ||
                            item.fontWeight === parseInt(fontWeight)) &&
                        (hasMimeType(mimeType, item.srcFormat) ||
                            (item.srcUrl && hasMimeType(mimeType, item.srcUrl)))
                );
            }
            return undefined;
        }
    ...
    Resource.canCompressImage = (filename) =>
    ...
    const REGEX_DATAURI$1 = new RegExp(
        `url\\("?(${STRING$1.DATAURI})"?\\),?\\s*`,
        "g"
    );
    ...
    function parseImageUrl(resourceHandler, baseMap, attr, styleSheetHref) {
        ...
        if (value && value !== "initial") {
            REGEX_DATAURI$1.lastIndex = 0;
            let result = value;
            let match;
            while ((match = REGEX_DATAURI$1.exec(value)) !== null) {
                if (match[2]) {
    ...
    const parseConditionText = (rule, value) => {
        var _a;
        return (
            ((_a = new RegExp(`\\s*@${rule}([^{]+)`).exec(value)) === null ||
            _a === void 0
                ? void 0
                : _a[1].trim()) || value
        );
    ...
    class Application {
        constructor(
            ...
            this._extensionManager = new (ExtensionManagerConstructor ||
                ExtensionManager)(this, cache);
        ...
        copyToDisk(directory, options) {
            ...
            return (
                ((_a = this.fileHandler) === null || _a === void 0
                    ? void 0
                    : _a.copyToDisk(directory, options)) ||
                frameworkNotInstalled()
            );
        }
        appendToArchive(pathname, options) {
            ...
            return (
                ((_a = this.fileHandler) === null || _a === void 0
                    ? void 0
                    : _a.appendToArchive(pathname, options)) ||
                frameworkNotInstalled()
            );
        }
        ...
        appendFromArchive(filename, options) {
            ...
            return (
                ((_a = this.fileHandler) === null || _a === void 0
                    ? void 0
                    : _a.appendFromArchive(filename, options)) ||
                frameworkNotInstalled()
            );
        }
        reset() {
        ...
        parseDocument(...elements) {
            ...
            const imageElements = [];
            const styleElement = insertStyleSheetRule(
            ...
            for (const element of this.rootElements) {
                element
                    ...
                    .forEach((source) => parseSrcSet(source.srcset));
                element
                    .querySelectorAll("video")
                    .forEach((source) => addImageSrc(source.poster));
                element
            ...
            if (preloadImages) {
                ...
                for (const [uri, data] of ASSET_RAWDATA.entries()) {
                    ...
                    if (
                        ...
                        const element = document.createElement("img");
                        element.src = `data:${mimeType};${
                            data.base64 ? `base64,${data.base64}` : data.content
                        }`;
                        const {
                            naturalWidth: width,
                    ...
                    .catch((error) => {
                        ...
                        const message =
                            target instanceof HTMLImageElement
                                ? target.src
                                : "";
                        return !this.userSettings.showErrorMessages ||
                            !isString(message) ||
        ...
        getDatasetName(attr, element) {
            return (
                element.dataset[attr + capitalize(this.systemName)] ||
            ...
            while (i < length) {
                ...
                if (childMap) {
                    ...
                    for (let j = 0; j < q; ++j) {
                        const k = j + 1;
                        result[k] =
                            ((_a = result[k]) === null || _a === void 0
                                ? void 0
                                : _a.concat(childMap[j])) || childMap[j];
                    }
                }
            }
            return result;
        ...
        applyStyleRule(item) {
            ...
            const styleSheetHref =
                ((_a = item.parentStyleSheet) === null || _a === void 0
                    ? void 0
                    : _a.href) || location.href;
            ...
            switch (item.type) {
                ...
                case CSSRule.STYLE_RULE: {
                    ...
                    parseSelectorText(item.selectorText, true).forEach(
                        (selectorText) => {
                            ...
                            while (i < length) {
                                ...
                                const styleData = getElementCache(
                                    element,
                                    attrStyle,
                                    sessionId
                                );
                ...
                case CSSRule.FONT_FACE_RULE: {
                    const attr =
                        (_b = REGEX_FONTFACE.exec(cssText)) === null ||
                        _b === void 0
                            ? void 0
                            : _b[1];
                    if (attr) {
                        const fontFamily = (
                            ((_c = REGEX_FONTFAMILY.exec(attr)) === null ||
                            _c === void 0
                                ? void 0
                                : _c[1]) || ""
                        ).trim();
                        const match = (
                            ((_d = REGEX_FONTSRC.exec(attr)) === null ||
                            _d === void 0
                                ? void 0
                                : _d[1]) || ""
                        ).split(XML$1.SEPARATOR);
                        if (fontFamily !== "" && match.length) {
                            const fontStyle =
                                ((_e = REGEX_FONTSTYLE.exec(attr)) === null ||
                                _e === void 0
                                    ? void 0
                                    : _e[1].toLowerCase()) || "normal";
                            const fontWeight = parseInt(
                                ((_f = REGEX_FONTWEIGHT.exec(attr)) === null ||
                                _f === void 0
                                    ? void 0
                                    : _f[1]) || "400"
                            );
                        ...
                        switch (rule.type) {
                            ...
                            case CSSRule.IMPORT_RULE: {
                                const uri = resolvePath(
                                    rule.href,
                                    ((_a = rule.parentStyleSheet) === null ||
                                    _a === void 0
                                        ? void 0
                                        : _a.href) || location.href
                                );
                                if (uri !== "") {
                                    this.resourceHandler.addRawData(
                                        uri,
                                        "text/css",
                                        "utf8",
                                        ""
    ...
    const { frameworkNotInstalled: frameworkNotInstalled$1 } = $lib$2.session;
    const {
        ...
        addAsset(data) {
            if (data.content || data.uri || data.base64) {
                ...
                const asset = this.assets.find(
                    (item) =>
                        item.pathname === pathname && item.filename === filename
                );
                if (asset) {
                    ...
                    if (asset) {
                        ...
                        return fetch(
                            this.hostname +
                                "/api/assets/copy" +
                                "?to=" +
                                encodeURIComponent(directory.trim()) +
                                "&empty=" +
                    ...
                    if (asset) {
                        ...
                        return fetch(
                            this.hostname +
                                ...
                                (
                                    options.format ||
                                    this.userSettings.outputArchiveFormat
                                )
                                    .trim()
                                    .toLowerCase() +
                                "&to=" +
    ...
    const { USER_AGENT, isUserAgent } = $lib$3.client;
    const {
        BOX_BORDER,
        CSS_UNIT,
        TEXT_STYLE,
    ...
    const { CHAR: CHAR$1, CSS: CSS$1, FILE: FILE$1, XML: XML$2 } = $lib$3.regex;
    const {
        actualClientRect,
        actualTextRangeRect,
        deleteElementCache,
    ...
    function setNaturalChildren() {
        ...
        if (this.naturalElement) {
            ...
            children = (
                ((_a = this.initial) === null || _a === void 0
                    ? void 0
                    : _a.children) || this.children
            ).slice(0);
        }
        this.naturalChildren = children;
        return children;
    }
    function setNaturalElements() {
        const children = this.naturalChildren.filter(
            (item) => item.naturalElement
        );
        this.naturalElements = children;
        return children;
    }
    function getFlexValue(attr, fallback, parent) {
    ...
    function hasTextAlign(value, localizedValue) {
        const textAlign = this.cssAscend(
            "textAlign",
            this.textElement && this.blockStatic && !this.hasPX("width")
        );
        return (
            (textAlign === value || textAlign === localizedValue) &&
            (this.blockStatic
                ? this.textElement &&
                  !this.hasPX("width", true, true) &&
                  !this.hasPX("maxWidth", true, true)
                : this.display.startsWith("inline"))
        );
    }
    function setDimension(styleMap, attr, attrMin, attrMax) {
        const valueA = styleMap[attr];
        const baseValue = this.parseUnit(valueA, attr);
        let value = Math.max(
            baseValue,
            this.parseUnit(styleMap[attrMin], attr)
        );
            ...
            switch (element.tagName) {
                ...
                case "EMBED": {
                    ...
                    if (size !== "") {
                        value = isNumber(size)
                            ? parseFloat(size)
                            : this.parseUnit(size, attr);
                        if (value > 0) {
                            this.css(
                                attr,
                                isPercent(size) ? size : size + "px"
                            );
                        }
                    }
                    break;
    ...
    function setOverflow() {
        ...
        if (
            ...
            const element = this.element;
            const { overflowX, overflowY } = this.cssAsObject(
                "overflowX",
                "overflowY"
            );
            if (
                this.hasHeight &&
                (this.hasPX("height") || this.hasPX("maxHeight")) &&
                (overflowY === "scroll" ||
                    (overflowY === "auto" &&
            ...
            if (
                (this.hasPX("width") || this.hasPX("maxWidth")) &&
                (overflowX === "scroll" ||
                    (overflowX === "auto" &&
    ...
    function convertBorderWidth(dimension, border) {
        if (!this.plainText) {
            ...
            const result = isLength(width, true)
                ? this.parseUnit(width, dimension)
                : convertFloat(this.style[border[1]]);
            if (result > 0) {
    ...
    function convertBox(attr, margin) {
        var _a;
        switch (this.display) {
            case "table":
                ...
                if (margin) {
                    switch (this.tagName) {
                        ...
                        case "TH":
                            return 0;
                        default: {
                            const parent = this.ascend({
                                condition: (item) => item.tagName === "TABLE",
                            })[0];
                            if (parent) {
                                const [horizontal, vertical] = parent
                                    .css("borderSpacing")
                                    .split(" ");
                                switch (attr) {
                                    case "marginTop":
                                    ...
                                    case "marginRight":
                                        if (
                                            this.actualParent.lastChild !== this
                                        ) {
    ...
    function canTextAlign() {
        return (
            this.naturalChild &&
            (this.inlineVertical || this.length === 0) &&
            !this.floating &&
            this.autoMargin.horizontal !== true
        );
    }
    function getInitialValue(attr, modified, computed) {
        return (
            (!this._preferInitial && this._styleMap[attr]) ||
            this.cssInitial(attr, modified, computed)
        );
    }
            ...
            for (const pseudo of pseudoList) {
                switch (pseudo) {
                    ...
                    case ":enabled":
                        if (
                            !child.inputElement ||
                            child.toElementBoolean("disabled")
                        ) {
                            return false;
                        }
                        break;
                    ...
                    case ":required":
                        if (
                            !child.inputElement ||
                            tagName === "BUTTON" ||
                            !child.toElementBoolean("required")
                        ) {
                            return false;
                        }
                        break;
                    ...
                    case ":default":
                        switch (tagName) {
                            ...
                            case "OPTION":
                                if (
                                    child.element.attributes["selected"] ===
                                    undefined
                                ) {
                                    return false;
                                }
                                ...
                                if (isNumber(placement)) {
                                    ...
                                    switch (placement) {
                                        ...
                                        case "odd":
                                            ...
                                            break;
                                        default: {
                                            const subMatch = REGEX_QUERY_NTH_CHILD_OFTYPE_VALUE.exec(
                                                placement
                                            );
                                            if (subMatch) {
                                                const modifier = convertInt(
                                                    subMatch[3]
                                                );
                                                if (subMatch[2]) {
                                                    if (subMatch[1]) {
    ...
    function convertPosition(attr) {
        if (!this.positionStatic) {
            ...
            if (isLength(unit)) {
                return this.parseUnit(
                    unit,
                    attr === "left" || attr === "right" ? "width" : "height"
                );
    ...
    class Node extends squared.lib.base.Container {
        ...
        init() {
            ...
            if (element) {
                ...
                const sessionId = this.sessionId;
                const styleMap =
                    getElementCache$1(element, "styleMap", sessionId) || {};
                let style;
                if (!this.pseudoElement) {
                    ...
                    if (styleElement) {
                        ...
                        if (length) {
                            ...
                            while (i < length) {
                                const attr = items[i++];
                                styleMap[
                                    convertCamelCase$1(attr)
                                ] = inline.getPropertyValue(attr);
                            }
                        }
                    }
                } else {
                    style = getStyle$1(
                        ...
                        getPseudoElt(element, sessionId)
                    );
                }
                if (styleElement) {
                    const revisedMap = {};
                    ...
                    switch (attr) {
                        ...
                        default:
                            if (attr.startsWith("margin")) {
                                ...
                                !/^(grid|flex|row|column|offset|scroll)/.test(
                                    attr
                                )
                            ) {
                                --length;
        ...
        intersectX(rect, dimension = "linear") {
            if (rect.width > 0) {
                ...
                return (
                    ...
                    (right >= Math.floor(leftA) &&
                        right <= Math.ceil(rightA)) ||
        ...
        intersectY(rect, dimension = "linear") {
            if (rect.height > 0) {
                ...
                return (
                    ...
                    (bottom >= Math.floor(topA) &&
                        bottom <= Math.ceil(bottomA)) ||
        ...
        withinX(rect, dimension = "linear") {
            if (this.pageFlow || rect.width > 0) {
                ...
                return (
                    aboveRange(bounds.left, rect.left) &&
                    belowRange(bounds.right, rect.right)
                );
            }
            return true;
        }
        withinY(rect, dimension = "linear") {
            if (this.pageFlow || rect.height > 0) {
                const bounds = this[dimension];
                return (
                    Math.ceil(bounds.top) >= rect.top &&
                    Math.floor(bounds.bottom) <= rect.bottom
                );
            }
            return true;
        }
        outsideX(rect, dimension = "linear") {
            if (this.pageFlow || rect.width > 0) {
                const bounds = this[dimension];
                return (
                    bounds.left < Math.floor(rect.left) ||
                    bounds.right > Math.ceil(rect.right)
                );
            }
            return false;
        }
        outsideY(rect, dimension = "linear") {
            if (this.pageFlow || rect.height > 0) {
                const bounds = this[dimension];
                return (
                    bounds.top < Math.floor(rect.top) ||
                    bounds.bottom > Math.ceil(rect.bottom)
                );
            }
            return false;
        }
        css(attr, value, cache = true) {
            ...
            return (
                this._styleMap[attr] ||
                (this.styleElement && this.style[attr]) ||
                ""
            );
        }
        ...
        cssInitial(attr, modified = false, computed = false) {
            var _a;
            let value =
                (((_a = this._initial) === null || _a === void 0
                    ? void 0
                    : _a.styleMap) || this._styleMap)[attr] ||
                (modified && this._styleMap[attr]);
            if (!value && computed) {
        ...
        cssSort(attr, ascending = true, duplicate = false) {
            return (duplicate ? this.duplicate() : this.children).sort(
                (a, b) => {
        ...
        cssTry(attr, value) {
            if (this.styleElement) {
                const element = this._element;
                return setStyleCache(
                    element,
                    attr,
                    this.sessionId,
                    value,
                    getStyle$1(element).getPropertyValue(attr)
                );
            }
            return false;
        }
        ...
        cssParent(attr, value, cache = false) {
            return this.naturalChild
                ? this.actualParent.css(attr, value, cache)
                : "";
        }
        cssCopy(node, ...attrs) {
            ...
            if (value) {
                switch (value) {
                    ...
                    default:
                        ...
                        if (type) {
                            return (
                                (hasBit$1(type, 2 /* LENGTH */) &&
                                    isLength(value)) ||
                                (hasBit$1(type, 4 /* PERCENT */) &&
                                    isPercent(value))
                            );
                        }
                        return true;
        ...
        setBounds(cache = true) {
            ...
            if (this.styleElement) {
                ...
                bounds = assignRect(
                    actualClientRect(this._element, this.sessionId)
                );
                this._bounds = bounds;
                        ...
                        if (query === "*") {
                            ...
                            while ((match = SELECTOR_G.exec(query)) !== null) {
                                ...
                                if (all) {
                                    ...
                                    while (
                                        (subMatch = SELECTOR_PSEUDO_CLASS.exec(
                                            segment
                                        )) !== null
                                    ) {
                        ...
                        if (selectors.length) {
                            ...
                            while (j < r) {
                                ...
                                if (
                                    (currentCount === 0 ||
                                        !result.includes(node)) &&
                                    ascendQuerySelector(0, dataEnd.adjacent, [
                                        node,
                                    ])
                                ) {
                                    result.push(node);
        ...
        get tagName() {
            ...
            if (result === undefined) {
                ...
                if (element) {
                    ...
                    result =
                        nodeName.charAt(0) === "#" ? nodeName : element.tagName;
                } else {
                    result = "";
                }
        ...
        get htmlElement() {
            ...
            if (result === undefined) {
                result =
                    !this.plainText && this._element instanceof HTMLElement;
        ...
        get svgElement() {
            ...
            if (result === undefined) {
                result =
                    (!this.htmlElement &&
                        !this.plainText &&
                        this._element instanceof SVGElement) ||
                    (this.imageElement &&
                        FILE$1.SVG.test(this.toElementString("src")));
        ...
        get textElement() {
            ...
            if (result === undefined) {
                result =
                    this.plainText || (this.inlineText && !this.inputElement);
        ...
        get bounds() {
            return (
                ...
                assignRect(this.boundingClientRect)
            );
        }
        get linear() {
            ...
            if (result === undefined) {
                if (
                    ...
                    result = {
                        alignSelf:
                            alignSelf === "auto"
                                ? this.cssParent("alignItems")
                                : alignSelf,
                        justifySelf:
                            justifySelf === "auto"
                                ? this.cssParent("justifyItems")
                                : justifySelf,
                ...
                if (isPercent(value)) {
                    if (this.pageFlow) {
                        result =
                            ((_a = this.actualParent) === null || _a === void 0
                                ? void 0
                                : _a.hasHeight) || this.documentBody;
                    } else {
                        result =
                ...
                if (!this.imageElement && !this.svgElement) {
                    ...
                    if (hasOwnStyle) {
                        ...
                        if (isPercent(lineHeight)) {
                            ...
                            if (
                                PX.test(lineHeight) &&
                                this._cssStyle.lineHeight !== "inherit"
                            ) {
                                const fontSize = getInitialValue.call(
                                    this,
                                    "fontSize"
                                );
                        ...
                        const parent = this.ascend({
                            condition: (item) => item.lineHeight > 0,
                        })[0];
                        if (parent) {
                    ...
                    if (
                        ...
                        this.multiline ||
                        (this.block &&
                            this.naturalChildren.some(
                                (node) => node.textElement
                            ))
                    ) {
                        result = value;
                    }
        ...
        get marginTop() {
            ...
            if (result === undefined) {
                result = this.inlineStatic
                    ? 0
                    : convertBox.call(this, "marginTop", true);
        ...
        get marginBottom() {
            ...
            if (result === undefined) {
                result = this.inlineStatic
                    ? 0
                    : convertBox.call(this, "marginBottom", true);
        ...
        get contentBox() {
            return (
                ...
                (this.tableElement && isUserAgent(8 /* FIREFOX */))
            );
        }
        get contentBoxWidth() {
            ...
            if (result === undefined) {
                result =
                    this.tableElement &&
                    this.css("borderCollapse") === "collapse"
                        ? 0
        ...
        get inline() {
            ...
            if (result === undefined) {
                ...
                result =
                    value === "inline" ||
                    (value === "initial" &&
                        !ELEMENT_BLOCK.includes(this.tagName));
        ...
        get inlineVertical() {
            ...
            if (result === undefined) {
                if (this.naturalElement && !this.floating) {
                    const value = this.display;
                    result =
                        value.startsWith("inline") || value === "table-cell";
                } else {
                    result = false;
                }
                    ...
                    case "inline":
                        if (
                            ...
                            result =
                                !this.hasPX("width") &&
                                convertFloat(
                                    getNamedItem(this._element, "width")
                                ) === 0;
                            break;
                        }
                    default:
        ...
        get blockStatic() {
            ...
            if (result === undefined) {
                result =
                    this.pageFlow &&
                    ((this.block && !this.floating) ||
                        this.lineBreak ||
                        (this.blockDimension &&
                            (getInitialValue.call(this, "width") === "100%" ||
                                getInitialValue.call(this, "minWidth") ===
                                    "100%") &&
                            !this.hasPX("maxWidth")));
        ...
        get centerAligned() {
            ...
            if (result === undefined) {
                if (!this.pageFlow) {
                    ...
                    result =
                        this.autoMargin.leftRight ||
                        (canTextAlign.call(this) &&
                            hasTextAlign.call(this, "center"));
                }
            ...
            if (result === undefined) {
                if (!this.pageFlow) {
                    ...
                    result =
                        ...
                        (canTextAlign.call(this) &&
                            hasTextAlign.call(this, "right", "end"));
                }
        ...
        get autoMargin() {
            ...
            if (result === undefined) {
                if (
                    ...
                    const styleMap = this._styleMap;
                    const left =
                        styleMap.marginLeft === "auto" &&
                        (this.pageFlow || this.hasPX("right"));
                    const right =
                        styleMap.marginRight === "auto" &&
                        (this.pageFlow || this.hasPX("left"));
                    const top =
                        styleMap.marginTop === "auto" &&
                        (this.pageFlow || this.hasPX("bottom"));
                    const bottom =
                        styleMap.marginBottom === "auto" &&
                        (this.pageFlow || this.hasPX("top"));
                    result = {
        ...
        get textContent() {
            ...
            if (result === undefined) {
                result =
                    this.naturalChild && !this.svgElement
                    ...
                    if (this.textElement) {
                        ...
                        if (length) {
                            ...
                            while (i < length) {
                                ...
                                if (node.textElement) {
                                    const rect = actualTextRangeRect(
                                        node.element,
                                        node.sessionId
                                    );
        ...
        get multiline() {
            ...
            if (result === undefined) {
                if (this.plainText) {
                    result =
                        Math.floor(getRangeClientRect(this._element).width) >
                        this.actualParent.box.width;
                } else if (
                    this.styleText &&
                    (this.inlineFlow || this.naturalElements.length === 0)
                ) {
                    result =
                ...
                switch (result) {
                    ...
                    default:
                        if (
                            ...
                            while (parent) {
                                const color = getInitialValue.call(
                                    parent,
                                    "backgroundColor",
                                    true
                                );
        ...
        get visibleStyle() {
            ...
            if (result === undefined) {
                if (this.plainText) {
                    ...
                    result = {
                        background:
                            borderWidth || backgroundImage || backgroundColor,
                        borderWidth,
                        backgroundImage,
                        backgroundColor,
                        backgroundRepeat:
        ...
        get actualParent() {
            ...
            if (result === undefined) {
                ...
                result =
                    (parentElement &&
                        getElementAsNode(parentElement, this.sessionId)) ||
                    null;
            ...
            if (result === undefined) {
                if (this.naturalChild && this.styleElement) {
                    ...
                    if (PX.test(value)) {
                        ...
                        result =
                            ((((_a = this.actualParent) === null ||
                            _a === void 0
                                ? void 0
                                : _a.fontSize) ||
                                getFontSize(getStyle$1(document.body))) *
                                parseFloat(value)) /
                            100;
                    } else {
                    ...
                    if (element && hasComputedStyle$1(element)) {
                        result =
                            ((_b = getElementAsNode(
                                ...
                                ? void 0
                                : _b.fontSize) ||
                            getFontSize(getStyle$1(element));
                    } else {
                        result =
                            ((_c = this.ascend({
                                condition: (item) => item.fontSize > 0,
                            })[0]) === null || _c === void 0
                                ? void 0
                                : _c.fontSize) ||
                            getFontSize(getStyle$1(document.body));
    ...
    (function (NODE_PROCEDURE) {
        ...
        NODE_PROCEDURE[(NODE_PROCEDURE["ACCESSIBILITY"] = 16)] =
            "ACCESSIBILITY";
        NODE_PROCEDURE[(NODE_PROCEDURE["LOCALIZATION"] = 32)] = "LOCALIZATION";
        NODE_PROCEDURE[(NODE_PROCEDURE["CUSTOMIZATION"] = 64)] =
            "CUSTOMIZATION";
    ...
    const { BOX_MARGIN, BOX_PADDING, BOX_POSITION } = $lib$4.css;
    const { isTextNode: isTextNode$1 } = $lib$4.dom;
    const { equal } = $lib$4.math;
    const { getElementAsNode: getElementAsNode$1 } = $lib$4.session;
    const {
    ...
    const { XML: XML$3 } = $lib$4.regex;
    const CSS_SPACING_KEYS = Array.from(CSS_SPACING.keys());
    const INHERIT_ALIGNMENT = [
        "position",
        "display",
        "verticalAlign",
        "float",
    ...
    function cascadeActualPadding(children, attr, value) {
        ...
        while (i < length) {
            ...
            if (item.blockStatic) {
                ...
                if (
                    item.has("lineHeight") &&
                    item.lineHeight > item.bounds.height
                ) {
                    return false;
                    ...
                    if (
                        !cascadeActualPadding(item.naturalChildren, attr, value)
                    ) {
                        return false;
                    } else {
                        valid = true;
    ...
    function parseExclusions(
        ...
        let offset = 0;
        const value =
            parentDataset[attr + "Child" + systemName] ||
            parentDataset[attr + "Child"];
        if (value) {
    ...
    function isBlockWrap() {
        return this.blockVertical || this.percentWidth > 0;
    }
    function checkBlockDimension(previous) {
        return (
            this.blockDimension &&
            Math.ceil(this.bounds.top) >= previous.bounds.bottom &&
            (isBlockWrap.call(this) || isBlockWrap.call(previous))
        );
    }
    function getPercentWidth() {
        return this.inlineDimension && !this.hasPX("maxWidth")
            ? this.percentWidth
            : -Infinity;
    }
    function getLayoutWidth() {
        return (
            this.actualWidth + Math.max(this.marginLeft, 0) + this.marginRight
        );
    }
    function getDatasetName(attr) {
        const dataset = this.dataset;
        return (
            dataset[attr + capitalize$1(this.localSettings.systemName)] ||
            dataset[attr]
        );
    }
    function applyBoxReset(boxReset, attrs, region, start, node) {
        for (let i = 0; i < 4; ++i) {
    ...
    function applyBoxAdjustment(boxAdjustment, attrs, region, start, node) {
        for (let i = 0; i < 4; ++i) {
            const key = CSS_SPACING_KEYS[i + start];
            if (hasBit$2(region, key)) {
                const previous = this.registerBox(key);
    ...
    class NodeUI extends Node {
        ...
        static outerRegion(node) {
            let top = Infinity,
                ...
                bottom = -Infinity,
                left = Infinity;
            let actualTop, actualRight, actualBottom, actualLeft;
            ...
            node.each((item) => {
                if (item.companion) {
                    ...
                    if (item.marginBottom < 0) {
                        const value =
                            actualBottom + Math.abs(item.marginBottom);
            ...
            if (result.length > 1) {
                result.sort((a, b) => {
                    ...
                    const heightA = a.baselineHeight + a.marginBottom,
                        heightB = b.baselineHeight + b.marginBottom;
        ...
        of(containerType, ...alignmentType) {
            return (
                this.is(containerType) &&
                alignmentType.some((value) => this.hasAlign(value))
            );
        }
        attr(name, attr, value, overwrite = true) {
            ...
            if (value) {
                ...
                return (
                    (obj === null || obj === void 0 ? void 0 : obj[attr]) || ""
                );
            }
        }
        unsafe(name, value) {
        ...
        lockedAttr(name, attr) {
            var _a;
            return (
                ((_a = this._locked[name]) === null || _a === void 0
                    ? void 0
                    : _a[attr]) === true
            );
        }
        ...
        hide(options) {
            if (
                options === null || options === void 0 ? void 0 : options.remove
            ) {
        ...
        inherit(node, ...modules) {
            modules.forEach((name) => {
                switch (name) {
                    ...
                    case "alignment": {
                        const styleMap = this._styleMap;
                        INHERIT_ALIGNMENT.forEach(
                            (attr) => (styleMap[attr] = node.css(attr))
                        );
                        if (!this.positionStatic) {
                    ...
                    case "styleMap":
                        this.cssCopyIfEmpty(
                            node,
                            ...Object.keys(node.unsafe("styleMap"))
                        );
                        break;
                    case "textStyle":
        ...
        setExclusions(systemName) {
            ...
            if (this.naturalElement) {
                ...
                if (hasKeys(dataset) || hasKeys(parentDataset)) {
                    systemName = capitalize$1(
                        systemName || this.localSettings.systemName
                    );
                    this.exclude({
                        resource: parseExclusions(
                            "excludeResource",
                            NODE_RESOURCE,
                            dataset,
                            parentDataset,
        ...
        removeTry(options) {
            ...
            if (renderParent) {
                ...
                if (renderTemplates) {
                    const index = renderChildren.findIndex(
                        (node) => node === this
                    );
                    if (index !== -1) {
                            ...
                            if (replaceWith) {
                                ...
                                if (replaceParent) {
                                    const replaceTemplates =
                                        replaceParent.renderTemplates;
                                    if (replaceTemplates) {
                                        const replaceIndex = replaceTemplates.findIndex(
                                            (item) => item.node === replaceWith
                                        );
                                        if (replaceIndex !== -1) {
                                            (_a = options.beforeReplace) ===
                                                null || _a === void 0
                                                ? void 0
                                                : _a.call(this, replaceWith);
                                            renderChildren[index] = replaceWith;
                                            renderTemplates[index] =
                                                replaceTemplates[replaceIndex];
                                            replaceTemplates.splice(
                                                replaceIndex,
                                                1
                                            );
                                            replaceParent.renderChildren.splice(
                                                replaceIndex,
                                                1
                                            );
                                ...
                                (_b =
                                    options === null || options === void 0
                                        ? void 0
                                        : options.beforeReplace) === null ||
                                _b === void 0
                                    ? void 0
                                    : _b.call(this, undefined);
        ...
        alignedVertically(siblings, cleared, horizontal) {
            if (this.lineBreak) {
                ...
                return siblings[siblings.length - 1].blockStatic
                    ? 1 /* VERTICAL */
                    : 0 /* HORIZONTAL */;
                ...
                if (isArray(siblings)) {
                    ...
                    if (cleared) {
                        if (
                            cleared.size &&
                            (cleared.has(this) ||
                                this.siblingsLeading.some(
                                    (item) => item.excluded && cleared.has(item)
                                ))
                        ) {
                            ...
                            if (floating && previous.floating) {
                                if (
                                    (horizontal &&
                                        this.float === previous.float) ||
                                    Math.floor(this.bounds.top) ===
                                        Math.floor(previous.bounds.top)
                                ) {
                                    return 0 /* HORIZONTAL */;
                                } else if (
                                    Math.ceil(this.bounds.top) >=
                                    previous.bounds.bottom
                                ) {
                                    if (
                                        siblings.every(
                                            (item) => item.inlineDimension
                                        )
                                    ) {
                                        ...
                                        if (
                                            ...
                                            const length = actualParent.naturalChildren.filter(
                                                (item) =>
                                                    item.visible &&
                                                    item.pageFlow
                                            ).length;
                                            if (
                                                ...
                                                let width =
                                                    actualParent.box.width -
                                                    getLayoutWidth.call(this);
                                                siblings.forEach(
                                                    (item) =>
                                                        (width -= getLayoutWidth.call(
                                                            item
                                                        ))
                                                );
                                                if (width >= 0) {
                ...
                for (const previous of this.siblingsLeading) {
                    if (previous.lineBreak) {
                        ...
                        (((floating && previous.childIndex === 0) ||
                            horizontal === false) &&
                            previous.plainText &&
                            previous.multiline)
                    ) {
                        ...
                        (this.blockStatic || this.display === "table") &&
                        (!previous.floating ||
                            (cleared === null || cleared === void 0
                                ? void 0
                                : cleared.has(previous)))
                    ) {
                        return 1 /* VERTICAL */;
        ...
        previousSiblings(options) {
            var _a, _b, _c;
            return traverseElementSibling(
                this.nodeGroup
                    ? (_b =
                          (_a = this.firstChild) === null || _a === void 0
                              ? void 0
                              : _a.element) === null || _b === void 0
                        ? void 0
                        : _b.previousSibling
                    : (_c = this.innerMostWrapped.element) === null ||
                      _c === void 0
                    ? void 0
                    : _c.previousSibling,
                "previousSibling",
            ...
            if (offset !== 0) {
                ...
                if (attr) {
                    ...
                    if (offset === undefined) {
                        ...
                        if (
                            !negative &&
                            (this._boxReset[attr] === 0 ? this[attr] : 0) +
                                boxAdjustment[attr] +
                                offset <=
                                0
                        ) {
                ...
                if (node) {
                    ...
                    if (adjustment !== undefined) {
                        ...
                        if (
                            ...
                            (boxReset[attr] === 0 ? this[attr] : 0) + value <= 0
                        ) {
                            value = 0;
        ...
        actualPadding(attr, value) {
            if (value > 0) {
                if (!this.layoutElement) {
                    ...
                    if (node.naturalChild) {
                        return canCascadeChildren.call(node) &&
                            cascadeActualPadding(
                                node.naturalChildren,
                                attr,
                                value
                            )
                            ? 0
        ...
        actualBoxWidth(value) {
            ...
            if (this.pageFlow && !this.documentRoot) {
                ...
                const screenWidth =
                    this.localSettings.screenDimension.width -
                    offsetLeft -
                    offsetRight;
                if (screenWidth > 0) {
        ...
        get element() {
            return (
                this._element ||
                (!!this.innerWrapped &&
                    this.innerMostWrapped.unsafe("element")) ||
                null
            );
        }
        ...
        get naturalChild() {
            ...
            if (result === undefined) {
                result = !!((_a = this._element) === null || _a === void 0
                    ? void 0
                    : _a.parentElement);
            ...
            if (result === undefined) {
                ...
                if (element) {
                    if (isTextNode$1(element)) {
                        ...
                        result =
                            "INPUT_" +
                            convertWord(element.type, true).toUpperCase();
                    } else {
                ...
                if (this.baseline) {
                    ...
                    if (children.length) {
                        result = children.every(
                            (node) => node.baselineElement && node.length === 0
                        );
                    } else {
                        result =
        ...
        get onlyChild() {
            ...
            return (
                ((_b =
                    (_a = this.renderParent) === null || _a === void 0
                        ? void 0
                        : _a.renderChildren.length) !== null && _b !== void 0
                    ? _b
        ...
        get textEmpty() {
            ...
            if (result === undefined) {
                if (
                    ...
                    result =
                        value === "" ||
                        (!this.preserveWhiteSpace && value.trim() === "");
                } else {
                    result = false;
                }
        ...
        set use(value) {
            const use = this.use;
            this.dataset["use" + capitalize$1(this.localSettings.systemName)] =
                (use ? use + ", " : "") + value;
        }
        get use() {
        ...
        get extensions() {
            ...
            if (result === undefined) {
                const use =
                    (_a = this.use) === null || _a === void 0
                        ? void 0
                        : _a.trim();
                result = use ? use.split(XML$3.SEPARATOR) : [];
    ...
    class LayoutUI extends squared.lib.base.Container {
        ...
        get itemCount() {
            var _a;
            return (_a = this._itemCount) !== null && _a !== void 0
                ? _a
                : this.length;
        }
        set type(value) {
                    ...
                    if (length > 1) {
                        ...
                        while (i < length) {
                            ...
                            if (
                                node.blockStatic ||
                                node.multiline ||
                                Math.ceil(node.bounds.top) >= previousBottom
                            ) {
                                result = false;
                                break;
                            }
    ...
    const {
        getNamedItem: getNamedItem$1,
        isTextNode: isTextNode$2,
        removeElementsByClassName,
    } = $lib$5.dom;
    const { maxArray } = $lib$5.math;
    const {
        appendSeparator,
    ...
    function getCounterIncrementValue(
        parent,
        counterName,
        pseudoElt,
        sessionId,
        ...
        return (
            counterIncrement &&
            getCounterValue(counterIncrement, counterName, fallback)
        );
    }
    ...
    function checkPseudoDimension(styleMap, after, absolute) {
        if (
            (after || convertFloat$1(styleMap.width) === 0) &&
            convertFloat$1(styleMap.height) === 0
        ) {
            for (const attr in styleMap) {
                if (
                    /(padding|Width|Height)/.test(attr) &&
                    convertFloat$1(styleMap[attr]) > 0
                ) {
                    return true;
                } else if (
                    !absolute &&
                    attr.startsWith("margin") &&
                    convertFloat$1(styleMap[attr]) !== 0
                ) {
                    return true;
                }
    ...
    const requirePadding = (node) =>
        node.textElement && (node.blockStatic || node.multiline);
    const getRelativeOffset = (node, fromRight) =>
        node.positionRelative
            ? node.hasPX("left")
                ? node.left * (fromRight ? 1 : -1)
                : node.right * (fromRight ? -1 : 1)
            : 0;
    const hasOuterParentExtension = (node) =>
        node.ascend({ condition: (item) => !!item.use }).length > 0;
    const setMapDepth = (map, depth, id, node) => {
        var _a;
        return (
            ((_a = map.get(depth)) === null || _a === void 0
                ? void 0
                : _a.set(id, node)) || map.set(depth, new Map([[id, node]]))
        );
    };
    ...
    class ApplicationUI extends Application {
            ...
            while (i < j) {
                ...
                if (node.documentRoot) {
                    if (node.renderChildren.length === 0 && !node.inlineText) {
                        ...
                        if (
                            naturalElement.length &&
                            naturalElement.every((item) => item.documentRoot)
                        ) {
                            continue;
                        }
                    }
                    const layoutName = node.innerMostWrapped.data(
            ...
            documentRoot.forEach((layout) => {
                ...
                if (renderTemplates) {
                    this.saveDocument(
                        layout.layoutName,
                        baseTemplate +
                            controllerHandler.cascadeDocument(
                                renderTemplates,
                                Math.abs(node.depth)
                            ),
                        node.dataset["pathname" + systemName],
                        (
                            (_a = node.renderExtension) === null ||
                            _a === void 0
                                ? void 0
        ...
        copyToDisk(directory, options) {
            return super.copyToDisk(
                directory,
                this.createAssetOptions(options)
            );
        }
        appendToArchive(pathname, options) {
            return super.appendToArchive(
                pathname,
                this.createAssetOptions(options)
            );
        }
        saveToArchive(filename, options) {
            return super.saveToArchive(
                filename,
                this.createAssetOptions(options)
            );
        }
        ...
        conditionElement(element, pseudoElt) {
            if (!this._excluded.has(element.tagName)) {
                if (
                    this.controllerHandler.visibleElement(element, pseudoElt) ||
                    this._cascadeAll
                ) {
                    return true;
                } else if (!pseudoElt) {
                    ...
                    if (
                        iterateArray$2(element.children, (item) =>
                            controllerHandler.visibleElement(item)
                        ) === Infinity
                    ) {
                        return true;
                    }
            ...
            if (isTextNode$2(element)) {
                if (
                    ...
                    ((parent === null || parent === void 0
                        ...
                        (parent.tagName !== "PRE" ||
                            parent.element.childElementCount === 0))
                ) {
        ...
        saveDocument(filename, content, pathname, index) {
            if (isString$3(content)) {
                const layout = {
                    pathname: pathname
                        ...
                        : appendSeparator(
                              ...
                              this._controllerSettings.layout.pathName
                          ),
                    filename,
                    content,
                    index,
                ...
                if (
                    index === undefined ||
                    !(index >= 0 && index < this._layouts.length)
                ) {
        ...
        addLayout(layout) {
            ...
            if (layout.containerType !== 0) {
                ...
                if (template) {
                    return this.addLayoutTemplate(
                        template.parent || layout.parent,
                        layout.node,
                        template,
                        layout.renderIndex
                    );
            ...
            if (template) {
                if (!node.renderExclude) {
                    if (node.renderParent) {
                        const renderTemplates = safeNestedArray(
                            parent,
                            "renderTemplates"
                        );
                        if (
                            ...
                            !(
                                index >= 0 &&
                                index < parent.renderChildren.length
                            )
                        ) {
        ...
        createCache(documentRoot) {
            ...
            if (node) {
                ...
                if (node.documentBody) {
                    ...
                    parent.exclude({
                        resource:
                            NODE_RESOURCE.FONT_STYLE |
                            NODE_RESOURCE.VALUE_STRING,
                ...
                cache.each((item) => {
                    if (!item.pseudoElement) {
                        item.setBounds(
                            !resetBounds && preAlignment[item.id] === undefined
                        );
                    } else {
                    ...
                    pseudoElements.forEach((item) => {
                        ...
                        if (item.pageFlow) {
                            ...
                            styleElement = insertStyleSheetRule$1(
                                `#${
                                    id +
                                    getPseudoElt$1(item.element, item.sessionId)
        ...
        afterCreateCache(node) {
            ...
            const filename =
                dataset["filename" + systemName] || dataset.filename;
            const iteration = dataset["iteration" + systemName];
            const prefix =
                (isString$3(filename) &&
                    filename.replace(
                        new RegExp(
                            `\\.${this._controllerSettings.layout.fileExtension}$`
                        ),
                        ""
                    )) ||
                node.elementId ||
            ...
            const layoutName = convertWord$1(
                suffix > 0 ? prefix + "_" + suffix : prefix,
                true
            );
        ...
        useElement(element) {
            ...
            return (
                isString$3(use) &&
                use
                    .split(XML$4.SEPARATOR)
                    .some((value) => !!this.extensionManager.retrieve(value))
            );
        }
        toString() {
            var _a;
            return (
                ((_a = this.layouts[0]) === null || _a === void 0
                    ? void 0
                    : _a.content) || ""
            );
        }
            ...
            if (
                ...
                if (depth === 0) {
                    ...
                    for (const name of node.extensions) {
                        if (
                            (_a = this.extensionManager.retrieve(name)) ===
                                null || _a === void 0
                                ? void 0
                                : _a.cascadeAll
                        ) {
                ...
                const controllerHandler = this.controllerHandler;
                if (
                    (node.excluded && !hasOuterParentExtension(node)) ||
                    controllerHandler.preventNodeCascade(parentElement)
                ) {
                    return node;
                }
                const sessionId = this.processing.sessionId;
                const beforeElement = this.createPseduoElement(
                    parentElement,
                    "::before",
                    sessionId
                );
                const afterElement = this.createPseduoElement(
                    parentElement,
                    "::after",
                    sessionId
                );
                ...
                while (i < length) {
                    ...
                    if (element === beforeElement) {
                        ...
                        if (extensions) {
                            prioritizeExtensions(
                                this.getDatasetName("use", element),
                                extensions
                            ).some((item) => item.init(element));
                        }
        ...
        setBaseLayout() {
            ...
            {
                let maxDepth = 0;
                setMapDepth(mapY, -1, 0, documentRoot.parent);
                cache.each((node) => {
                    if (node.length) {
                ...
                let i = 0;
                while (i < maxDepth) {
                    mapY.set(getMapIndex(i++), new Map());
                }
                    ...
                    if (cascade && node.length) {
                        node.cascade((item) => {
                            ...
                            if (item.length) {
                                ...
                                (_a = mapY.get(depth)) === null || _a === void 0
                                    ? void 0
                                    : _a.delete(item.id);
                                setMapDepth(
                                    mapY,
                    ...
                    for (let i = 0; i < length; ++i) {
                        ...
                        if (
                            ...
                            (parentY.alignmentType === 0 ||
                                parentY.hasAlign(2 /* UNKNOWN */) ||
                                nodeY.hasAlign(8192 /* EXTENDABLE */)) &&
                            ...
                            traverse: {
                                ...
                                for (; l < length; ++l, ++m) {
                                    ...
                                    if (item.pageFlow) {
                                        ...
                                        if (m === 0) {
                                            const next =
                                                item.siblingsTrailing[0];
                                            if (next) {
                                                if (
                                                    !isHorizontalAligned(
                                                        item
                                                    ) ||
                                                    next.alignedVertically([
                                                        item,
                                                    ]) > 0
                                                ) {
                                                    vertical.push(item);
                            ...
                            if (q > 1) {
                                ...
                                if (q > 1) {
                                    ...
                                    segEnd = vertical[q - 1];
                                    if (
                                        isHorizontalAligned(segEnd) &&
                                        segEnd !== axisY[length - 1]
                                    ) {
                        ...
                        if (
                            !nodeY.rendered &&
                            nodeY.hasSection(APP_SECTION.EXTENSION)
                        ) {
                            const descendant = extensionMap.get(nodeY.id);
                            let combined = descendant
                                ? renderExtension
                                    ? renderExtension.concat(descendant)
                                    : descendant
                                : renderExtension;
                            let next = false;
                            if (combined) {
                                ...
                                while (j < q) {
                                    ...
                                    if (ext.is(nodeY)) {
                                        if (
                                            ...
                                            (!descendant ||
                                                !descendant.includes(ext))
                                        ) {
                                            const result = ext.processNode(
                                                nodeY,
                                                parentY
                                            );
                                            if (result) {
                        ...
                        if (
                            !nodeY.rendered &&
                            nodeY.hasSection(APP_SECTION.RENDER)
                        ) {
                            let layout = this.createLayoutControl(
                                parentY,
                                nodeY
                            );
                            if (layout.containerType === 0) {
                                const result = nodeY.length
                                    ? controllerHandler.processUnknownParent(
                                          layout
                                      )
                                    : controllerHandler.processUnknownChild(
                                          layout
                                      );
                                if (result.next) {
                                    continue;
        ...
        processFloatHorizontal(layout) {
            ...
            const {
                containerType,
                alignmentType,
            } = controllerHandler.containerTypeVertical;
            ...
            layout.each((node, index) => {
                ...
                if (!clearedFloat) {
                    if (float === "left") {
                        ...
                        if (
                            leftAbove.some(
                                (item) => top >= item.bounds.bottom
                            ) ||
                            rightAbove.some((item) => top >= item.bounds.bottom)
                        ) {
            ...
            if (
                rightAbove.length +
                    ((rightBelow === null || rightBelow === void 0
                        ? void 0
                        : rightBelow.length) || 0) ===
                layout.length
            ) {
            ...
            if (inlineBelow.length) {
                ...
                const wrapper = controllerHandler.createNodeGroup(
                    node,
                    inlineBelow,
                    { parent }
                );
            ...
            layerIndex.forEach((item) => {
                ...
                if (Array.isArray(item[0])) {
                    ...
                    grouping.sort((a, b) =>
                        a.childIndex < b.childIndex ? -1 : 1
                    );
                    const node = layout.node;
                    if (node.layoutVertical) {
                        floatgroup = node;
                    } else {
                        floatgroup = controllerHandler.createNodeGroup(
                            grouping[0],
                            grouping,
                            { parent: node }
                        );
                ...
                segments.forEach((seg) => {
                    ...
                    const target = controllerHandler.createNodeGroup(
                        first,
                        seg,
                        { parent: node, delegate: true }
                    );
                    ...
                    if (seg.some((child) => child.percentWidth > 0)) {
                        ...
                        group.setContainerType(
                            containerType,
                            alignmentType |
                                (group.unknownAligned ? 2 /* UNKNOWN */ : 0)
                        );
                    } else {
        ...
        processFloatVertical(layout) {
            ...
            if (layout.containerType !== 0) {
                const node = layout.node;
                const parent = controllerHandler.createNodeGroup(node, [node], {
                ...
                for (let i = 0; i < length; ++i) {
                    ...
                    if (floatedRows[i] === null && pageFlow.length) {
                        ...
                        this.addLayout(
                            new LayoutUI(
                                node,
                                controllerHandler.createNodeGroup(
                                    pageFlow[0],
                                    pageFlow,
                                    { parent: node }
                                ),
                        ...
                        if (pageFlow.length || floating.length) {
                            const basegroup = controllerHandler.createNodeGroup(
                                floating[0] || pageFlow[0],
                                [],
                                { parent: node }
                            );
                            ...
                            let subgroup;
                            if (floating.length) {
                                const floatgroup = controllerHandler.createNodeGroup(
                                    floating[0],
                                    floating,
                                    { parent: basegroup }
                                );
                            ...
                            if (pageFlow.length) {
                                subgroup = controllerHandler.createNodeGroup(
                                    pageFlow[0],
                                    pageFlow,
                                    { parent: basegroup }
                                );
                            ...
                            children.forEach((item) => {
                                if (!item.nodeGroup) {
                                    item = controllerHandler.createNodeGroup(
                                        item,
                                        [item],
                                        { parent: basegroup, delegate: true }
                                    );
                                }
                                this.addLayout(
                                    new LayoutUI(
                            ...
                            if (pageFlow.length && floating.length) {
                                const [leftAbove, rightAbove] = partitionArray(
                                    floating,
                                    (item) => item.float !== "right"
                                );
                                this.setFloatPadding(
        ...
        createPseduoElement(element, pseudoElt, sessionId) {
            let styleMap = getElementCache$2(
                element,
                `styleMap${pseudoElt}`,
                sessionId
            );
            ...
            if (element.tagName === "Q") {
                ...
                if (!content) {
                    content =
                        getStyle$2(element, pseudoElt).getPropertyValue(
                            "content"
                        ) ||
                        (pseudoElt === "::before"
                            ? "open-quote"
            ...
            if (styleMap) {
                ...
                if (value) {
                    ...
                    if (textContent.trim() === "") {
                        ...
                        let current = element;
                        do {
                            value = getStyle$2(current).getPropertyValue(
                                "content"
                            );
                            if (value !== "inherit") {
                                break;
                    ...
                    switch (value) {
                        ...
                        default: {
                            ...
                            if (url !== "") {
                                ...
                                let match;
                                while (
                                    (match = REGEX_COUNTER.exec(value)) !== null
                                ) {
                                    const attr = match[1];
                                    if (attr) {
                                        content += getNamedItem$1(
                                            element,
                                            attr.trim()
                                        );
                    ...
                    if (content || value === '""') {
                        if (tagName === "") {
                            tagName = /^(inline|table)/.test(styleMap.display)
                                ? "span"
                                : "div";
                        }
                        const pseudoElement = createPseudoElement(
                            element,
                            tagName,
                            pseudoElt === "::before" ? 0 : -1
                        );
                        if (tagName === "img") {
                            pseudoElement.src = content;
                            const image = this.resourceHandler.getImage(
                                content
                            );
                            if (image) {
                                if (
                                    !isString$3(styleMap.width) &&
                                    image.width > 0
                                ) {
                                    styleMap.width = formatPX$1(image.width);
                                }
                                if (
                                    !isString$3(styleMap.height) &&
                                    image.height > 0
                                ) {
        ...
        setFloatPadding(parent, target, inlineAbove, leftAbove, rightAbove) {
            ...
            inlineAbove.forEach((child) => {
                ...
                if (child.blockStatic) {
                    paddingNodes = paddingNodes.concat(
                        child.cascade((item) => requirePadding(item))
                    );
                }
            });
            ...
            if (leftAbove.length) {
                ...
                if (floatPosition !== -Infinity) {
                    ...
                    paddingNodes.forEach((child) => {
                        if (
                            Math.floor(child.linear.left) <= floatPosition ||
                            child.centerAligned
                        ) {
                    ...
                    if (marginLeft !== -Infinity) {
                        ...
                        if (offset > 0 && offset < boxWidth) {
                            target.modifyBox(
                                ...
                                offset +
                                    (!spacing &&
                                    target.find((child) => child.multiline, {
                                        cascade: true,
                                    })
                                        ? Math.max(
                                              marginLeft,
                                              this._controllerSettings
                                                  .deviations
            ...
            if (rightAbove.length) {
                ...
                rightAbove.forEach((child) => {
                    if (child.bounds.top < bottom) {
                        const left =
                            child.linear.left + getRelativeOffset(child, true);
                ...
                if (floatPosition !== Infinity) {
                    ...
                    paddingNodes.forEach((child) => {
                        if (
                            child.multiline ||
                            Math.ceil(child.linear.right) >= floatPosition
                        ) {
            ...
            return this._layouts.sort((a, b) => {
                ...
                if (indexA !== indexB) {
                    if (
                        indexA === 0 ||
                        indexB === Infinity ||
                        (indexB === undefined && !(indexA === Infinity))
                    ) {
                        return -1;
                    } else if (
                        indexB === 0 ||
    ...
    const {
        ...
        isUserAgent: isUserAgent$1,
        isWinEdge,
    } = $lib$6.client;
    const {
        BOX_BORDER: BOX_BORDER$1,
        BOX_PADDING: BOX_PADDING$1,
        formatPX: formatPX$2,
        getStyle: getStyle$3,
        isLength: isLength$1,
        isPercent: isPercent$1,
    } = $lib$6.css;
    const { isTextNode: isTextNode$3, withinViewport } = $lib$6.dom;
    const {
        capitalize: capitalize$3,
        convertFloat: convertFloat$2,
    ...
    const { pushIndent, pushIndentArray } = $lib$6.xml;
    ...
    function setBorderStyle(styleMap, defaultColor) {
        if (
            !isString$4(styleMap.border) &&
            !(
                BOX_BORDER$1[0][0] in styleMap ||
                BOX_BORDER$1[1][0] in styleMap ||
                BOX_BORDER$1[2][0] in styleMap ||
                BOX_BORDER$1[3][0] in styleMap
            )
        ) {
    ...
    function setButtonStyle(styleMap, applied, defaultColor) {
        ...
        if (
            !isString$4(styleMap.padding) &&
            !BOX_PADDING$1.some((attr) => !!styleMap[attr])
        ) {
    ...
    const getNumberValue = (style, attr) =>
        parseInt(style.getPropertyValue(attr));
    class ControllerUI extends Controller {
        ...
        applyDefaultStyles(element) {
            ...
            if (isTextNode$3(element)) {
                ...
                styleMap =
                    getElementCache$3(element, "styleMap", sessionId) || {};
                const tagName = element.tagName;
                if (isUserAgent$1(8 /* FIREFOX */)) {
                    switch (tagName) {
                        case "BODY":
                            if (
                                styleMap.backgroundColor === "rgba(0, 0, 0, 0)"
                            ) {
                ...
                switch (tagName) {
                    case "INPUT": {
                        ...
                        switch (type) {
                            ...
                            case "date":
                            case "datetime-local":
                                styleMap.paddingTop = formatPX$2(
                                    convertFloat$2(styleMap.paddingTop) + 1
                                );
                                styleMap.paddingRight = formatPX$2(
                                    convertFloat$2(styleMap.paddingRight) + 1
                                );
                                styleMap.paddingBottom = formatPX$2(
                                    convertFloat$2(styleMap.paddingBottom) + 1
                                );
                                styleMap.paddingLeft = formatPX$2(
                                    convertFloat$2(styleMap.paddingLeft) + 1
                                );
                                break;
                            default: {
                    ...
                    case "SELECT":
                        setBorderStyle(
                            styleMap,
                            this.localSettings.style.inputBorderColor
                        );
                        break;
                    case "BODY": {
                        const backgroundColor = styleMap.backgroundColor;
                        if (
                            !isString$4(backgroundColor) ||
                    ...
                    case "LI":
                        if (!isString$4(styleMap.listStyleImage)) {
                            ...
                            styleMap.listStyleImage = style.getPropertyValue(
                                "list-style-image"
                            );
                        }
                        break;
        ...
        includeElement(element) {
            return (
                !(
                    this._unsupportedTagName.has(element.tagName) ||
                    (element.tagName === "INPUT" &&
                        this._unsupportedTagName.has(
                            element.tagName + ":" + element.type
                        ))
        ...
        visibleElement(element, pseudoElt) {
            ...
            if (pseudoElt) {
                const parentElement = element.parentElement;
                style = parentElement
                    ? getStyle$3(parentElement, pseudoElt)
                    : getStyle$3(element);
                width = 1;
                height = 1;
            } else {
            ...
            if (!positionAbsolute(style)) {
                return (
                    ...
                    (style.getPropertyValue("display") === "block" &&
                        (getNumberValue(style, "margin-top") !== 0 ||
                            getNumberValue(style, "margin-bottom") !== 0))
                );
            }
            return false;
        }
            ...
            cache.each((node) => {
                if (!node.documentRoot && !node.pageFlow) {
                        ...
                        case "absolute": {
                            ...
                            if (absoluteParent) {
                                ...
                                if (node.autoPosition) {
                                    ...
                                    this.userSettings.supportNegativeLeftTop
                                ) {
                                    let outside = false;
                                    while (
                                        parent &&
                                        ...
                                        if (!outside) {
                                            if (
                                                ...
                                                const outsideX =
                                                    !overflowX &&
                                                    node.outsideX(linear);
                                                const outsideY =
                                                    !overflowY &&
                                                    node.outsideY(linear);
                                                if (
                                                    (outsideX &&
                                                        (node.left < 0 ||
                                                            node.right > 0)) ||
                    ...
                    if (parent !== actualParent) {
                        ...
                        if (
                            (absoluteParent === null ||
                            absoluteParent === void 0
                                ? void 0
                                : absoluteParent.positionRelative) &&
                            parent !== absoluteParent
                        ) {
            ...
            (cache || this.cache).sort((a, b) => {
                if (a.depth !== b.depth) {
                    ...
                    const parentA = a.documentParent,
                        parentB = b.documentParent;
                    if (parentA !== parentB) {
                        const depthA = parentA.depth,
                            depthB = parentB.depth;
                        if (depthA !== depthB) {
                            return depthA < depthB ? -1 : 1;
                        } else if (
                            parentA.actualParent === parentB.actualParent
                        ) {
            ...
            while (i < length) {
                ...
                switch (item.type) {
                    case 1 /* XML */: {
                        ...
                        const next = depth + 1;
                        const previous =
                            node.depth < 0 ? depth + node.depth : depth;
                        const beforeInside = this.getBeforeInsideTemplate(
                            id,
                            next
                        );
                        const afterInside = this.getAfterInsideTemplate(
                            id,
                            next
                        );
                        let template =
        ...
        setElementDimension(element, tagName, styleMap, attr, opposing) {
            ...
            if (!isString$4(dimension) || dimension === "auto") {
                const match = new RegExp(`\\s+${attr}="([^"]+)"`).exec(
                    element.outerHTML
                );
                if (match) {
                    const value = match[1];
                    ...
                    if (value && isLength$1(value)) {
                        ...
                        if (
                            !isString$4(styleMap[attrMax]) ||
                            !isPercent$1(attrMax)
                        ) {
                            const image = this.application.resourceHandler.getImage(
                                element.src
                            );
                            if (image && image.width > 0 && image.height > 0) {
                                styleMap[attr] = formatPX$2(
                                    (image[attr] * parseFloat(value)) /
                                        image[opposing]
                                );
                            }
                        }
    ...
    class ExtensionUI extends Extension {
        ...
        included(element) {
            return includes(
                this.application.getDatasetName("use", element),
                this.name
            );
        }
    ...
    class NodeGroupUI extends NodeUI {
        ...
        previousSiblings(options) {
            var _a;
            const node = (((_a = this._initial) === null || _a === void 0
                ? void 0
                : _a.children) || this.children)[0];
            return (
                (node === null || node === void 0
                    ? void 0
                    : node.previousSiblings(options)) || []
            );
        }
        nextSiblings(options) {
            var _a;
        ...
        get blockStatic() {
            ...
            if (result === undefined) {
                ...
                result =
                    this.some(
                        (node) => node.blockStatic || node.percentWidth > 0
                    ) ||
                    documentParent.percentWidth > 0 ||
                    (this.layoutVertical &&
                        (documentParent.hasWidth ||
                            this.some(
                                (node) =>
                                    node.centerAligned || node.rightAligned
                            ))) ||
                    (documentParent.blockStatic &&
                        (documentParent.layoutVertical ||
                            this.hasAlign(256 /* COLUMN */)));
        ...
        get floating() {
            ...
            if (result === undefined) {
                result = this.every(
                    (node) => node.floating || node.hasAlign(512 /* FLOAT */)
                );
                this._cached.floating = result;
            }
            return result;
        ...
        get childIndex() {
            ...
            if (result === Infinity) {
                this.each(
                    (node) => (result = Math.min(node.childIndex, result))
                );
        ...
        get containerIndex() {
            ...
            if (result === Infinity) {
                this.each(
                    (node) => (result = Math.min(node.containerIndex, result))
                );
    ...
    const {
        ...
        isUserAgent: isUserAgent$2,
    } = $lib$7.client;
    const { parseColor } = $lib$7.color;
    const {
        BOX_BORDER: BOX_BORDER$2,
        calculate,
        convertAngle,
        formatPX: formatPX$3,
    ...
    const { getNamedItem: getNamedItem$2 } = $lib$7.dom;
    const {
        cos,
        equal: equal$1,
        hypotenuse,
    ...
    const REGEX_BACKGROUNDIMAGE = new RegExp(
        `(?:initial|url\\([^)]+\\)|(repeating-)?(linear|radial|conic)-gradient\\(((?:to\\s+[a-z\\s]+|(?:from\\s+)?-?[\\d.]+(?:deg|rad|turn|grad)|(?:circle|ellipse)?\\s*(?:closest-side|closest-corner|farthest-side|farthest-corner)?)?(?:\\s*(?:(?:-?[\\d.]+(?:[a-z%]+)?\\s*)+)?(?:at\\s+[\\w %]+)?)?),?\\s*((?:${STRING_COLORSTOP})+)\\))`,
        "g"
    );
    ...
    function parseColorStops(node, gradient, value) {
        ...
        switch (gradient.type) {
            case "linear": {
                ...
                switch (angle) {
                    ...
                    default: {
                        size =
                            Math.abs(width * sin(angle - 180)) +
                            Math.abs(height * cos(angle - 180));
            ...
            if (color) {
                ...
                if (gradient.type === "conic") {
                    ...
                    if (isPercent$2(unit)) {
                        ...
                        offset =
                            (horizontal
                                ? node.parseWidth(unit, false)
                                : node.parseHeight(unit, false)) / size;
                    } else if (isCalc(unit)) {
                        offset =
                            calculate(match[6], {
                                boundingSize: size,
        ...
        if (repeat) {
            ...
            result.push(
                Object.assign(Object.assign({}, result[length - 1]), {
                    offset: 1,
                })
            );
        }
    ...
    function replaceWhiteSpace(node, value) {
        ...
        if (node.onlyChild && node.htmlElement) {
            ...
            if (
                (_c = node.nextSibling) === null || _c === void 0
                    ? void 0
                    : _c.blockStatic
            ) {
    ...
    function getBackgroundSize(node, index, value, screenDimension) {
        if (value) {
            ...
            return ResourceUI.getBackgroundSize(
                node,
    ...
    function setBorderStyle$1(node, boxStyle, attr, border) {
        ...
        if (style !== "none") {
            let width = formatPX$3(
                attr !== "outline"
                    ? node[border[1]]
                    : convertFloat$3(node.style[border[1]])
            );
            if (width !== "0px") {
    ...
    const getGradientPosition = (value) =>
        isString$5(value)
            ? value.includes("at ")
                ? /(.+?)?\s*at (.+?)\s*$/.exec(value)
                : [value, value]
    ...
    class ResourceUI extends Resource {
                ...
                switch (item.tagName) {
                    ...
                    case "OPTGROUP": {
                        const [
                            groupStringArray,
                            groupNumberArray,
                        ] = this.getOptionArray(item, showDisabled);
                        if (groupStringArray) {
        ...
        static parseBackgroundImage(node, backgroundImage, screenDimension) {
            ...
            if (backgroundImage !== "") {
                ...
                let match;
                while (
                    (match = REGEX_BACKGROUNDIMAGE.exec(backgroundImage)) !==
                    null
                ) {
                    const value = match[0];
                    if (value.startsWith("url(") || value === "initial") {
                        ...
                        const imageDimension = getBackgroundSize(
                            node,
                            i,
                            node.css("backgroundSize"),
                            screenDimension
                        );
                        const dimension = NodeUI.refitScreen(
                            node,
                            imageDimension || node.actualDimension
                        );
                        let gradient;
                        switch (type) {
                            case "linear": {
                                ...
                                switch (direction) {
                                    ...
                                    default:
                                        if (direction) {
                                            angle =
                                                getAngle(direction, 180) || 360;
                                        }
                                        break;
                                }
                                let x = truncateFraction(
                                        offsetAngleX(angle, width)
                                    ),
                                    y = truncateFraction(
                                        offsetAngleY(angle, height)
                                    );
                                if (
                                    x !== width &&
                                    y !== height &&
                                    !equal$1(Math.abs(x), Math.abs(y))
                                ) {
                                    let opposite;
                                    if (angle <= 90) {
                                        opposite = relativeAngle(
                                            { x: 0, y: height },
                                            { x: width, y: 0 }
                                        );
                                    } else if (angle <= 180) {
                                ...
                                linear.colorStops = parseColorStops(
                                    node,
                                    linear,
                                    match[4]
                                );
                            ...
                            case "radial": {
                                ...
                                [
                                    ...
                                    const length = Math.round(
                                        hypotenuse(
                                            Math.abs(corner[0] - left),
                                            Math.abs(corner[1] - top)
                                        )
                                    );
                                    closestCorner = Math.min(
                                        length,
                                        closestCorner
                                    );
                                ...
                                [width - left, height - top, left].forEach(
                                    (side) => {
                                        closestSide = Math.min(
                                            side,
                                            closestSide
                                        );
                                        farthestSide = Math.max(
                                            side,
                                            farthestSide
                                        );
                                    }
                                ...
                                radial.colorStops = parseColorStops(
                                    node,
                                    radial,
                                    match[4]
                                );
                            ...
                            case "conic": {
                                ...
                                conic.colorStops = parseColorStops(
                                    node,
                                    conic,
                                    match[4]
                                );
            ...
            if (node.plainText) {
                ...
                if (
                    (parent === null || parent === void 0
                        ...
                        : parent.preserveWhiteSpace) &&
                    parent.ascend({
                        condition: (item) => item.tagName === "PRE",
                        startSelf: true,
                    }).length
                ) {
                    ...
                    if (
                        nextSibling === null || nextSibling === void 0
                            ? void 0
                            : nextSibling.naturalElement
                    ) {
                        const textContent = node.textContent;
                        if (textContent.trim() !== "") {
                            const match = REGEX_TRAILINGINDENT.exec(
                                textContent
                            );
                            if (match) {
                                if (!nextSibling.textElement) {
                                    nextSibling = nextSibling.find(
                                        (item) =>
                                            item.naturalChild &&
                                            item.textElement,
                                        {
        ...
        setBoxStyle(node) {
            var _a;
            if (
                (node.styleElement || node.visibleStyle.background) &&
                node.hasResource(NODE_RESOURCE.BOX_STYLE)
            ) {
                const boxStyle = node.cssAsObject(
                    "backgroundSize",
                    "backgroundRepeat",
                    "backgroundPositionX",
                    "backgroundPositionY"
                ...
                setBorderStyle$1(node, boxStyle, "outline", BOX_BORDER$2[4]);
                if (node.hasResource(NODE_RESOURCE.IMAGE_SOURCE)) {
                    boxStyle.backgroundImage = ResourceUI.parseBackgroundImage(
                        node,
        ...
        setValueString(node) {
            ...
            if (element) {
                ...
                switch (element.tagName) {
                    case "INPUT":
                        ...
                        switch (element.type) {
                            ...
                            case "checkbox": {
                                ...
                                if (
                                    (companion === null || companion === void 0
                                        ? void 0
                            ...
                            case "submit":
                                if (
                                    value === "" &&
                                    !node.visibleStyle.backgroundImage
                                ) {
                                    value = "Submit";
                                }
                                break;
                            ...
                            case "tel":
                                ...
                                break;
                            case "file":
                                value = isUserAgent$2(8 /* FIREFOX */)
                                    ? "Browse..."
                                    : "Choose File";
                                break;
                            case "color": {
                                const borderColor = this.controllerSettings
                                    .style.inputColorBorderColor;
                                const backgroundColor = (
                                    parseColor(value) ||
                                    parseColor("rgb(0, 0, 0)")
                                ).valueAsRGBA;
                                const { width, height } = node.actualDimension;
                                const backgroundSize = `${width - 10}px ${
                                    height - 10
                                }px, ${width - 8}px ${height - 8}px`;
                    ...
                    default: {
                        ...
                        if (node.plainText || node.pseudoElement) {
                            ...
                            [value, inlined, trimming] = replaceWhiteSpace(
                                node,
                                replaceAmpersand(textContent)
                            );
                            inlined = true;
                ...
                if (value !== "") {
                    if (trimming && node.pageFlow) {
                        ...
                        if (value.length > 1) {
                            if (
                                ...
                                (previousSibling.plainText &&
                                    CHAR$2.TRAILINGSPACE.test(
                                ...
                                if (length) {
                                    previousSpaceEnd =
                                        textContent.charCodeAt(length - 1) ===
                                        32;
                                }
                            }
                        }
                        if (inlined) {
                            const trailingSpace =
                                ...
                                CHAR$2.TRAILINGSPACE.test(value);
                            if (
                                CHAR$2.LEADINGSPACE.test(value) &&
                            ...
                            if (trailingSpace) {
                                const nextSibling = node.siblingsTrailing.find(
                                    (item) => !item.excluded || item.lineBreak
                                );
                                if (
                                    (nextSibling === null ||
                                    nextSibling === void 0
                                    ...
                                    (previousSibling.block ||
                                        ...
                                        (previousSpaceEnd &&
                                            ...
                                            previousSibling.textContent.length >
                                                1) ||
                                        (node.multiline &&
                                            ResourceUI.hasLineBreak(node)))
                                    ? ""
                                    : STRING_SPACE
                            );
                            value = value.replace(
                                CHAR$2.TRAILINGSPACE,
                    ...
                    if (value !== "") {
                        node.data(ResourceUI.KEY_NAME, "valueString", {
                            key,
                            value,
                        });
                    }
        ...
        removeExcludedFromText(node, element) {
            const styled =
                element.children.length > 0 || element.tagName === "CODE";
                ...
                if (
                    ...
                    if (child) {
                        if (styled && child.htmlElement) {
                            const outerHTML = child.toElementString(
                                "outerHTML"
                            );
                            if (child.lineBreak) {
                                value = value.replace(
                                    !preserveWhitespace
                                        ? new RegExp(`\\s*${outerHTML}\\s*`)
                                        : outerHTML,
                                    "\\n"
                                );
                            ...
                            const textContent = child.plainText
                                ? child.textContent
                                : child[attr];
            ...
            if (styled) {
                value = value
                    .replace(/^\\n\\n/, "\\n")
                    .replace(/\\n\\n$/, "\\n")
                    .replace(ESCAPE.ENTITY, (match, capture) =>
                        String.fromCharCode(parseInt(capture))
                    );
            }
            return value;
        }
        ...
        processNode(node, parent) {
            ...
            node.each((item) => {
                ...
                if (item.css("columnSpan") === "all") {
                    ...
                    if (
                        item.textElement &&
                        ((_a = item.textBounds) === null || _a === void 0
                            ? void 0
                            : _a.overflow)
                    ) {
                        maxSize = NaN;
            ...
            const getColumnSizing = () =>
                isNaN(columnCount) && columnWidth > 0
    ...
    const REGEX_NAMED = new RegExp(
        `\\s*(repeat\\(\\s*(auto-fit|auto-fill|\\d+),\\s+(.+)\\)|${STRING_NAMED}|${STRING_MINMAX}|${STRING_FIT_CONTENT}|${STRING_UNIT}\\s*)\\s*`,
        "g"
    );
    const REGEX_REPEAT = new RegExp(
        `\\s*(${STRING_NAMED}|${STRING_MINMAX}|${STRING_FIT_CONTENT}|${STRING_UNIT})\\s*`,
        "g"
    );
    ...
    function setFlexibleDimension(dimension, gap, count, unit, max) {
        ...
        if (percent < 1 && fractional > 0) {
            const ratio =
                (dimension * percent -
                    (count - 1) * gap -
                    max.reduce((a, b) => a + Math.max(0, b), 0) -
                    filled) /
                fractional;
            if (ratio > 0) {
                for (i = 0; i < length; ++i) {
    ...
    const convertLength = (node, value, index) =>
        isLength$4(value)
            ? formatPX$4(
                  node.parseUnit(value, index !== 0 ? "width" : "height")
              )
            : value;
    class CssGrid extends ExtensionUI {
        ...
        processNode(node) {
            ...
            const [rowA, colA, rowB, colB] = horizontal
                ? [0, 1, 2, 3]
                : [1, 0, 3, 2];
            const rowData = [];
            const openCells = [];
            ...
            [
                node.cssInitial("gridTemplateRows", true),
                node.cssInitial("gridTemplateColumns", true),
                            ...
                            case 1:
                                if (command.startsWith("[")) {
                                    match[4]
                                        .split(CHAR$3.SPACE)
                                        .forEach((attr) =>
                                            safeNestedArray$2(name, attr).push(
                                                i
                                            )
                                        );
                                    ...
                                    if (iterations > 0) {
                                        ...
                                        while (
                                            ...
                                            let namedMatch;
                                            if (
                                                (namedMatch = REGEX_CELL_NAMED.exec(
                                                    subPattern
                                                )) !== null
                                            ) {
                                                ...
                                                (namedMatch = REGEX_CELL_MINMAX.exec(
                                                    subPattern
                                                )) !== null
                                            ) {
                                                repeating.push({
                                                ...
                                                (namedMatch = REGEX_CELL_FIT_CONTENT.exec(
                                                    subPattern
                                                )) !== null
                                            ) {
                                                repeating.push({
                                                ...
                                                (namedMatch = REGEX_CELL_UNIT.exec(
                                                    subPattern
                                                )) !== null
                                            ) {
                                                repeating.push({
                                    ...
                                    unit.push(
                                        convertLength(node, match[6], index)
                                    );
                                    unitMin.push(
                                        convertLength(node, match[5], index)
                                    );
                                    repeat.push(false);
                                    ++i;
                                } else if (command.startsWith("fit-content")) {
                                    unit.push(
                                        convertLength(node, match[7], index)
                                    );
                                    unitMin.push("0px");
                                    repeat.push(false);
                                    ++i;
                                    ...
                                    unit.push(
                                        convertLength(node, command, index)
                                    );
                                    unitMin.push("");
                                    repeat.push(false);
                                    ++i;
            ...
            if (
                ...
                node.every(
                    (item) =>
                        item.css("gridRowStart") === "auto" &&
                        item.css("gridColumnStart") === "auto"
                )
            ) {
                const [
                    directionA,
                ...
                node.each((item, index) => {
                    if (
                        !previous ||
                        item.linear[directionA] >=
                            previous.linear[directionB] ||
                    ...
                    const { gridRowEnd, gridColumnEnd } = item.cssAsObject(
                        "gridRowEnd",
                        "gridColumnEnd"
                    );
                    let rowSpan = 1;
                                ...
                                .forEach((area, j) => {
                                    if (area.charAt(0) !== ".") {
                                        const templateArea =
                                            templateAreas[area];
                                        if (templateArea) {
                                            templateArea.rowSpan =
                                                i - templateArea.rowStart + 1;
                                            templateArea.columnSpan =
                                                j -
                                                templateArea.columnStart +
                                                1;
                                        } else {
                                            templateAreas[area] = {
                                                rowStart: i,
                ...
                node.each((item, index) => {
                    const positions = item.cssAsTuple(
                        "gridRowStart",
                        "gridColumnStart",
                        "gridRowEnd",
                        "gridColumnEnd"
            ...
            let ITERATION;
            {
                const [data, outerCoord] = horizontal
                    ? [column, node.box.top]
                    : [row, node.box.left];
                let unit = data.unit;
                let length = 1;
                ...
                layout.forEach((item) => {
                    if (item) {
                        const [totalSpan, start, end] = horizontal
                            ? [item.columnSpan, 1, 3]
                            : [item.rowSpan, 0, 2];
                ...
                ITERATION = Math.max(
                    length,
                    outerCount,
                    (horizontal && !autoWidth) || (!horizontal && !autoHeight)
                        ? q
                        : 0
                );
                ...
                if (percent < 1) {
                    if (fr > 0) {
                        ...
                        for (let i = 0; i < q; ++i) {
                            ...
                            if (CssGrid.isFr(value)) {
                                unit[i] =
                                    percent * (parseFloat(value) / fr) + "fr";
                            }
                        }
                    } else if (auto === 1) {
                        const index = unit.findIndex(
                            (value) => value === "auto"
                        );
                        if (index !== -1) {
            ...
            node.each((item, index) => {
                ...
                const [ROW_SPAN, COLUMN_SPAN] = horizontal
                    ? [rowSpan, columnSpan]
                    : [columnSpan, rowSpan];
                while (placement[0] === 0 || placement[1] === 0) {
                    ...
                    if (PLACEMENT[rowA] === 0) {
                        ...
                        for (
                            let i = dense ? 0 : getOpenRowIndex(openCells),
                                j = 0,
                                k = -1;
                            i < length;
                            ++i
                        ) {
                            const l = getOpenCellIndex(
                                ITERATION,
                                COLUMN_SPAN,
                                openCells[i]
                            );
                    ...
                    if (node.cssTry("display", "block")) {
                        node.each((item) => {
                            ...
                            item.data(
                                CSS_GRID,
                                "boundsData",
                                Object.assign(Object.assign({}, item.bounds), {
                                    width,
                                    height,
                                })
                            );
    ...
    CssGrid.isAligned = (node) =>
        node.hasHeight &&
        /^space-|center|flex-end|end/.test(node.css("alignContent"));
    CssGrid.isJustified = (node) =>
        (node.blockStatic || node.hasWidth) &&
        /^space-|center|flex-end|end|right/.test(node.css("justifyContent"));
    ...
    class Flexbox extends ExtensionUI {
        static createDataAttribute(node, children) {
            return Object.assign(Object.assign({}, node.flexdata), {
                rowCount: 0,
                columnCount: 0,
                children,
            });
        ...
        processNode(node) {
            ...
            const [children, absolute] = node.partition(
                (item) => item.pageFlow
            );
            ...
            if (mainData.wrap) {
                ...
                if (length > 1) {
                    ...
                    while (i < length) {
                        ...
                        const group = controller.createNodeGroup(seg[0], seg, {
                            parent: node,
                            delegate: true,
                            cascade: true,
                        });
    ...
    function getRowIndex(columns, target) {
        ...
        while (i < length) {
            const index = columns[i++].findIndex((item) => {
                ...
                return (
                    withinRange$3(topA, top) ||
                    (Math.ceil(topA) >= top &&
                        topA < Math.floor(item.bounds.bottom))
                );
            });
            if (index !== -1) {
                return index;
                    ...
                    return (
                        node.every(
                            (item) =>
                                ...
                                item.every(
                                    (child) => child.display === "table-cell"
                                )
                        ) || node.every((item) => item.display === "table-cell")
                    );
                } else if (
                    node.percentWidth === 0 ||
                    !node.find((item) => item.percentWidth > 0, {
                        cascade: true,
                    })
                ) {
                    let minLength = false;
                    ...
                    return (
                        ...
                        (minLength &&
                            node.every(
                                (item) =>
                                    item.length > 0 &&
                                    NodeUI.linearData(item.children).linearX
                            ))
                    );
                }
            }
        ...
        processNode(node) {
                ...
                if (columnLength !== -1) {
                    ...
                    for (let i = 0; i < length; ++i) {
                        ...
                        for (let j = 0; j < q; ++j) {
                            ...
                            if (
                                i === 0 ||
                                aboveRange$1(left, columnRight[i - 1])
                            ) {
                    ...
                    const maxColumn = columns.reduce(
            ...
            if (columnCount > 1 && columns[0].length === node.length) {
                    ...
                    for (let j = 0, start = 0, spacer = 0; j < rowCount; ++j) {
                        ...
                        if (!item["spacer"]) {
                            ...
                            if (columnEnd.length) {
                                ...
                                item.actualParent.naturalChildren.forEach(
                                    (sibling) => {
                                        if (
                                            !assigned.has(sibling) &&
                                            sibling.visible &&
                                            !sibling.rendered
                                        ) {
                                            const {
                ...
                children.forEach((group) =>
                    group.forEach((item) => (item.parent = node))
                );
                if (
                    node.tableElement &&
                    node.css("borderCollapse") === "collapse"
                ) {
    ...
    const { convertListStyle: convertListStyle$1 } = $lib$9.css;
    const { isNumber: isNumber$3 } = $lib$9.util;
    const hasSingleImage = (visibleStyle) =>
        visibleStyle.backgroundImage && !visibleStyle.backgroundRepeat;
        ...
        condition(node) {
            ...
            if (length) {
                ...
                for (let i = 0; i < length; ++i) {
                    ...
                    if (
                        (item.display === "list-item" &&
                            (type !== "none" || item.innerBefore)) ||
                        (item.marginLeft < 0 &&
                            type === "none" &&
                            hasSingleImage(item.visibleStyle))
                    ) {
                        bulletVisible = true;
                    }
    ...
    const {
        convertFloat: convertFloat$4,
        withinRange: withinRange$4,
    } = $lib$a.util;
    const TRANSLATE_OPTIONS = { relative: true };
    class Relative extends ExtensionUI {
        is(node) {
            return (
                (node.positionRelative && !node.autoPosition) ||
                convertFloat$4(node.verticalAlign) !== 0
            );
        }
        condition() {
            return true;
        ...
        postOptimize(node) {
            ...
            const verticalAlign = !node.baselineAltered
                ? convertFloat$4(node.verticalAlign)
                : 0;
            let top = 0,
                right = 0,
                bottom = 0,
            ...
            if (renderParent.support.positionTranslation) {
                ...
                if (
                    ...
                    const index = renderParent.renderChildren.findIndex(
                        (item) => item === node
                    );
                    if (index !== -1) {
                    ...
                    if (node.parseUnit(node.css("textIndent")) < 0) {
                        ...
                        renderParent.renderEach((item) => {
                            if (item.alignSibling("topBottom") === documentId) {
                                ...
                                item.alignSibling("bottomTop") === documentId
                            ) {
                                item.alignSibling(
                                    "bottomTop",
                                    target.documentId
                    ...
                    if (node.baselineActive && !node.baselineAltered) {
                        for (const children of renderParent.horizontalRows || [
                            renderParent.renderChildren,
                        ]) {
                    ...
                    while (i < length) {
                        ...
                        if (item === node) {
                            ...
                            item.positionRelative &&
                            item.renderParent === renderParent
                        ) {
                            preceding = true;
                        }
                        if (item.pageFlow) {
    ...
    class Sprite extends ExtensionUI {
        ...
        condition(node) {
            ...
            if (
                backgroundImage !== "" &&
                (!node.use || this.included(node.element))
            ) {
                const image =
                    this.resource.getRawData(backgroundImage) ||
                    this.resource.getImage(resolveURL$1(backgroundImage));
                if (image) {
                    ...
                    if (
                        ...
                        node.data(EXT_NAME.SPRITE, "mainData", {
                            image,
                            position,
                        });
                        return true;
    ...
    const { getNamedItem: getNamedItem$3 } = $lib$b.dom;
    const { maxArray: maxArray$1 } = $lib$b.math;
    const {
        isNumber: isNumber$4,
        replaceMap,
    ...
    function setAutoWidth(node, td, data) {
        data.percent =
            Math.round((td.bounds.width / node.box.width) * 100) + "%";
        data.expand = true;
    }
    ...
    const setBoundsWidth = (node) =>
        node.css("width", formatPX$5(node.bounds.width), true);
    class Table extends ExtensionUI {
        ...
        processNode(node) {
            ...
            const [horizontal, vertical] = borderCollapse
                ? [0, 0]
                : replaceMap(
                      node.css("borderSpacing").split(" "),
                      (value, index) =>
                          index === 0
                              ? node.parseWidth(value)
                              : node.parseHeight(value)
                  );
            const spacingWidth =
                horizontal > 1 ? Math.round(horizontal / 2) : horizontal;
            const spacingHeight =
                vertical > 1 ? Math.round(vertical / 2) : vertical;
            ...
            for (let i = 0; i < rowCount; ++i) {
                ...
                tr.each((td, index) => {
                    ...
                    if (
                        !td.visibleStyle.backgroundImage &&
                        !td.visibleStyle.backgroundColor
                    ) {
                        if (colgroup) {
                            const {
                                backgroundImage,
                                backgroundColor,
                            } = getStyle$4(colgroup.children[index + 1]);
                            ...
                            if (
                                backgroundColor &&
                                !REGEX_BACKGROUND$2.test(backgroundColor)
                            ) {
                    ...
                    const reevaluate =
                        mapWidth[j] === undefined || mapWidth[j] === "auto";
                                ...
                                (width === mapBounds[j] &&
                                    ((length && percent) ||
                                        (percent &&
                                            ...
                                            td.parseWidth(columnWidth) >=
                                                td.parseWidth(mapWidth[j])) ||
                                        (length &&
                    ...
                    if (spacingWidth > 0) {
                        td.modifyBox(
                            16 /* MARGIN_LEFT */,
                            j === 0 ? horizontal : spacingWidth
                        );
                        td.modifyBox(
                            4 /* MARGIN_RIGHT */,
                            index === 0 ? spacingWidth : horizontal
                        );
                    }
                    if (spacingHeight > 0) {
                        td.modifyBox(
                            2 /* MARGIN_TOP */,
                            i === 0 ? vertical : spacingHeight
                        );
                        td.modifyBox(
                            8 /* MARGIN_BOTTOM */,
                            i + rowSpan < rowCount ? spacingHeight : vertical
                        );
                    }
            ...
            if (
                node.hasPX("width", false) &&
                mapWidth.some((value) => isPercent$4(value))
            ) {
            ...
            mainData.layoutType = (() => {
                if (mapWidth.length > 1) {
                    mapPercent = mapWidth.reduce(
                        (a, b) => a + (isPercent$4(b) ? parseFloat(b) : 0),
                        0
                    );
                    if (
                        mainData.layoutFixed &&
                        mapWidth.reduce(
                            (a, b) => a + (isLength$5(b) ? parseFloat(b) : 0),
                            0
                        ) >= node.actualWidth
                    ) {
                        return 4 /* COMPRESS */;
                        ...
                        (mapWidth.length > 1 &&
                            mapWidth.some((value) => isPercent$4(value))) ||
                        mapWidth.every(
                            (value) => isLength$5(value) && value !== "0px"
                        )
                    ) {
                        return 3 /* VARIABLE */;
                    } else if (
                        mapWidth.every((value) => value === mapWidth[0])
                    ) {
                        if (
                            node.find((td) => td.hasHeight, { cascade: true })
                        ) {
                            mainData.expand = true;
                            return 3 /* VARIABLE */;
                            ...
                            if (node.hasWidth) {
                                ...
                                const td = node.cascade(
                                    (item) => item.tagName === "TD"
                                );
                                return td.length &&
                                    td.every((item) =>
                                        withinRange$5(
                ...
                if (!caption.hasWidth) {
                    if (caption.textElement) {
                        if (!caption.hasPX("maxWidth")) {
                            caption.css(
                                "maxWidth",
                                formatPX$5(caption.bounds.width)
                            );
                        }
                        ...
                        switch (mainData.layoutType) {
                            ...
                            case 3 /* VARIABLE */:
                                if (columnWidth === "auto") {
                                    ...
                                    isLength$5(columnWidth) &&
                                    parseInt(columnWidth) > 0
                                ) {
                                    if (
                                        td.bounds.width >= parseInt(columnWidth)
                                    ) {
                                        setBoundsWidth(td);
                                        data.expand = false;
            ...
            if (mainData.borderCollapse) {
                const borderTop = node.cssAsObject(
                    "borderTopColor",
                    "borderTopStyle",
                    "borderTopWidth"
                );
                const borderRight = node.cssAsObject(
                    "borderRightColor",
                    "borderRightStyle",
                    "borderRightWidth"
                );
                const borderBottom = node.cssAsObject(
                    "borderBottomColor",
                    "borderBottomStyle",
                    "borderBottomWidth"
                );
                const borderLeft = node.cssAsObject(
                    "borderLeftColor",
                    "borderLeftStyle",
                    "borderLeftWidth"
                );
                const borderTopWidth = parseInt(borderTop.borderTopWidth),
                    borderRightWidth = parseInt(borderRight.borderRightWidth),
                    borderBottomWidth = parseInt(
                        borderBottom.borderBottomWidth
                    ),
                    borderLeftWidth = parseInt(borderLeft.borderLeftWidth);
                let hideTop = false,
                    hideRight = false,
                    hideBottom = false,
                    hideLeft = false;
    ...
    const { isLength: isLength$6 } = $lib$c.css;
    ...
    class VerticalAlign extends ExtensionUI {
        ...
        condition(node) {
            ...
            while (i < length) {
                ...
                if (
                    !(
                        item.positionStatic ||
                        (item.positionRelative && item.length)
                    )
                ) {
                    return false;
            ...
            return (
                ...
                isNaN(sameValue) &&
                inlineVertical > 1 &&
                NodeUI.linearData(children).linearX
            );
        }
        processNode(node) {
            node.each((item) => {
                if (
                    (item.inlineVertical && isLength$6(item.verticalAlign)) ||
                    item.imageElement ||
                    item.svgElement
                ) {
        ...
        postConstraints(node) {
            if (node.layoutHorizontal) {
                for (const children of node.horizontalRows || [
                    node.renderChildren,
                ]) {
                    const aboveBaseline = [];
                    let minTop = Infinity;
                            ...
                            baseline.setBox(2 /* MARGIN_TOP */, {
                                ...
                                adjustment:
                                    ...
                                    Math.min(
                                        0,
                                        above.parseHeight(
                                            above.cssInitial("verticalAlign")
                                        )
                                    ),
                            });
    ...
    const { formatPX: formatPX$6 } = $lib$d.css;
    const { maxArray: maxArray$2 } = $lib$d.math;
    const { getElementCache: getElementCache$4 } = $lib$d.session;
    const { iterateReverseArray } = $lib$d.util;
    const DOCTYPE_HTML =
        ((_a = document.doctype) === null || _a === void 0
            ? void 0
            : _a.name) === "html";
    const COLLAPSE_TOP = [
        "marginTop",
        ...
        if (!direction || isBlockElement(child, true)) {
            ...
            if (node[borderWidth] === 0) {
                if (node[paddingName] === 0) {
                    ...
                    if (offsetParent >= 0 && offsetChild >= 0) {
                        ...
                        if (resetChild) {
                            ...
                            if (!direction && target.floating) {
                                ...
                                target.actualParent.naturalChildren.forEach(
                                    (item) => {
                                        if (
                                            item.floating &&
                                            item !== target &&
                                            item.intersectY(bounds, "bounds")
                                        ) {
                            ...
                            if (
                                height === 0 &&
                                !target.every(
                                    (item) => item.floating || !item.pageFlow
                                )
                            ) {
                                resetBox(
                                    target,
                    ...
                    let blockAll = true;
                    do {
                        const endChild = direction
                            ? child.firstStaticChild
                            : child.lastStaticChild;
                        if (
                            endChild &&
            ...
            if (direction !== undefined) {
                if (direction) {
                    ...
                    return (
                        ...
                        validAboveChild(firstChild, false)
                    );
                } else {
                    const lastChild = node.lastStaticChild;
                    return (
                        isBlockElement(lastChild) &&
                        validBelowChild(lastChild, false)
                    );
                }
            }
        }
    ...
    function getBottomChild(node) {
        ...
        if (!node.floatContainer) {
            ...
            if (
                ...
                (node.hasHeight &&
                    Math.floor(bottomChild.linear.bottom) < node.box.bottom)
            ) {
                ...
                if (item.floating) {
                    if (!bottomChild) {
                        ...
                        if (bottomFloatChild) {
                            ...
                            Math.ceil(item.linear.bottom) >= node.box.bottom
                        ) {
                            bottomFloatChild = item;
                        }
                    } else if (
                        item.linear.bottom >= bottomChild.linear.bottom
                    ) {
                        bottomChild = item;
                        break;
                    }
                    ...
                    if (
                        bottomFloatChild &&
                        bottomFloatChild.linear.bottom > item.linear.bottom
                    ) {
    ...
    function isVerticalOverflow(node) {
        for (const value of [
            node.cssInitial("overflowX"),
            node.cssInitial("overflowY"),
        ]) {
            switch (value) {
                case "auto":
                case "hidden":
    ...
    const setMinHeight = (node, offset) =>
        node.css(
            ...
            formatPX$6(
                Math.max(
                    offset,
                    node.hasPX("minHeight", false)
                        ? node.parseHeight(node.css("minHeight"))
                        : 0
                )
            )
        );
    const canResetChild = (node, children = true) =>
        ((!children && node.blockStatic) ||
            (children && node.length > 0 && !node.floating)) &&
        !node.layoutElement &&
        !node.tableElement &&
        node.tagName !== "FIELDSET";
    const validAboveChild = (node, children) =>
        !node.hasHeight &&
        node.borderBottomWidth === 0 &&
        node.paddingBottom === 0 &&
        canResetChild(node, children);
    const validBelowChild = (node, children) =>
        !node.hasHeight &&
        node.borderTopWidth === 0 &&
        node.paddingTop === 0 &&
        canResetChild(node, children);
    const validSibling = (node) =>
        node.pageFlow &&
        node.blockDimension &&
        !node.floating &&
        !node.excluded;
        ...
        afterBaseLayout() {
            ...
            this.cacheProcessing.each((node) => {
                ...
                if (
                    ...
                    const collapseMargin =
                        pageFlow &&
                        isBlockElement(node, true) &&
                        ...
                        if (current.pageFlow) {
                            if (collapseMargin) {
                                if (!current.floating) {
                                    ...
                                    if (
                                        current.linear.bottom >=
                                        lastChild.linear.bottom
                                    ) {
                                        lastChild = current;
                                    }
                                } else {
                                ...
                                if (i > 0) {
                                    ...
                                    if (q) {
                                        ...
                                        if (isBlockElement(previous, false)) {
                                            ...
                                            while (
                                                validAboveChild(inherit, true)
                                            ) {
                                                let bottomChild = getBottomChild(
                                                    inherit
                                                );
                                                if (
                                                    (bottomChild === null ||
                                            ...
                                            inherit = current;
                                            while (
                                                validBelowChild(inherit, true)
                                            ) {
                                                let topChild =
                                                    inherit.firstStaticChild;
                                                if (
                                                    isBlockElement(
                                                        topChild,
                                                        true
                                            ...
                                            if (inheritedBottom) {
                                                ...
                                                if (
                                                    marginBottom >= adjacentTop
                                                ) {
                                                    (
                                                        previous.registerBox(
                                                            8 /* MARGIN_BOTTOM */
                                                        ) || previous
                                                    ).setCacheValue(
                                                        "marginBottom",
                    ...
                    if (previousSiblings.length) {
                        ...
                        if (nextSiblings.length) {
                            ...
                            if (above.rendered && below.rendered) {
                                const inline =
                                    above.inlineStatic && below.inlineStatic;
                                ...
                                if (
                                    !above.multiline &&
                                    above.has("lineHeight")
                                ) {
                                    const aboveOffset = Math.floor(
                                        (above.lineHeight -
                                            above.bounds.height) /
                                            2
                                    );
                                ...
                                if (
                                    !below.multiline &&
                                    below.has("lineHeight")
                                ) {
                                    const belowOffset = Math.round(
                                        (below.lineHeight -
                                            below.bounds.height) /
                                            2
                                    );
                                ...
                                if (inline) {
                                    ...
                                    if (previousSiblings.length === 1) {
                                        aboveLineBreak = aboveLineBreak.lineBreak
                                            ? node
                                            : undefined;
                                    }
                                    aboveLineBreak === null ||
                                    aboveLineBreak === void 0
                                        ? void 0
                                ...
                                if (aboveParent !== belowParent) {
                                    ...
                                    while (
                                        belowParent &&
                                        belowParent !== actualParent
                                    ) {
                                        below = belowParent;
                                        belowParent = below.renderParent;
                                    }
                                }
                                [offset, below] = getMarginOffset(
                                    below,
                                    above,
                                    lineHeight,
                                    aboveLineBreak
                                ...
                                if (offset >= 1) {
                                    if (
                                        (below.lineBreak || below.excluded) &&
                                        actualParent.lastChild === below
                                    ) {
                                        actualParent.modifyBox(
                                            128 /* PADDING_BOTTOM */,
                                            offset
                                        );
                                        valid = true;
                            ...
                            if (
                                ...
                                const rect =
                                    previousStart.bounds.height === 0 &&
                                    previousStart.length
                                        ? NodeUI.outerRegion(previousStart)
                                        : previousStart.linear;
                                const offset =
                                    actualParent.box.bottom -
                                    (previousStart.lineBreak ||
                                    previousStart.excluded
                                        ? rect.top
                                        : rect.bottom);
                                if (offset !== 0) {
                                    if (
                                        previousStart.rendered ||
                                        actualParent.visibleStyle.background
                                    ) {
                                        actualParent.modifyBox(
                                            128 /* PADDING_BOTTOM */,
                                            offset
                                ...
                                const offset =
                                    (nextStart.lineBreak || nextStart.excluded
                                        ? nextStart.linear.bottom
                                        : nextStart.linear.top) -
                                    actualParent.box.top;
                                if (offset !== 0) {
                                    if (
                                        nextStart.rendered ||
                                        actualParent.visibleStyle.background
                ...
                if (
                    ...
                    if (
                        (renderParent === null || renderParent === void 0
                            ? void 0
                            : renderParent.hasAlign(4 /* AUTO_LAYOUT */)) ===
                        false
                    ) {
                        if (node.blockDimension && !node.floating) {
                            if (renderParent.layoutVertical) {
                                ...
                                const index = children.findIndex(
                                    (item) => item === outerWrapper
                                );
                                if (index !== -1) {
                                    if (
                                        !node.lineBreakLeading &&
                                        !node.baselineAltered
                                    ) {
                                ...
                                if (
                                    ...
                                    found: {
                                        ...
                                        for (let i = 0; i < length; ++i) {
                                            ...
                                            row.forEach((item) => {
                                                const innerWrapped =
                                                    item.innerMostWrapped;
                                                if (
                                                    validSibling(innerWrapped)
                                                ) {
                                                    maxBottom = Math.max(
                                                        innerWrapped.actualRect(
                                                            "bottom"
                                                        ),
                                                        maxBottom
                                                    );
                                    ...
                                    renderParent.layoutHorizontal ||
                                    renderParent.hasAlign(1024 /* INLINE */)
                                ) {
                                ...
                                if (horizontal) {
                                    ...
                                    horizontal.forEach((item) => {
                                        if (item.nodeGroup) {
                                            actualChildren = actualChildren.concat(
                                                item.cascade(
                                                    (child) =>
                                                        child.naturalChild
                                                )
                                    ...
                                    const naturalChildren =
                                        parent.naturalChildren;
                        ...
                        if (
                            ...
                            const documentId = outerWrapper.alignSibling(
                                "leftRight"
                            );
                            if (documentId !== "") {
                                ...
                                while (true) {
                                    const siblingsLeading =
                                        current.siblingsLeading;
                                    if (
                                        siblingsLeading.length &&
                    ...
                    node.naturalChildren.forEach((item) => {
                        ...
                        if (!item.floating) {
                            if (floating.length) {
                                ...
                                if (renderParent) {
                                    const [reset, adjustment] = item.getBox(
                                        2 /* MARGIN_TOP */
                                    );
                                    const marginTop =
                                        (reset === 0 ? item.marginTop : 0) +
                                        adjustment;
                                    if (marginTop > 0) {
                                        ...
                                        while (i < length) {
                                            ...
                                            if (
                                                ...
                                                let floatingRenderParent =
                                                    previous.outerMostWrapper
                                                        .renderParent;
