            ...
            for (let i = 0, length = pseudoList.length; i < length; ++i) {
                ...
                switch (pseudo) {
                    ...
                    case ':empty': {
                        ...
                        if (childNodes.length && iterateArray$1(childNodes, item => item.nodeName[0] !== '#' || item.nodeName === '#text') === Infinity) {
                            return false;
                        }
                        break;
                    }
        ...
        querySelectorAll(value, sorted = true, customMap) {
            ...
            if (queryMap) {
                ...
                for (const query of parseSelectorText(value)) {
                    native: {
                        const items = splitEnclosing$2(query, CSS$1.SELECTOR_ENCLOSING_G);
                        let selector = '', expand;
