...
export default class NegativeViewport<T extends View> extends squared.base.ExtensionUI<T> {
    public condition(node: T, parent: T) {
        return !node.pageFlow && parent.naturalElement && parent.documentRoot && (
            ...
            Math.floor(node.linear.bottom) > Math.ceil(parent.box.bottom) && (node.top > 0 || node.marginTop > 0 || !node.hasPX($const.CSS.TOP) && node.bottom < 0) && parent.hasPX($const.CSS.HEIGHT)
        );
    }
