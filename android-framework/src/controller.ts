            ...
            switch (node.element.tagName) {
                case 'IMG': {
                    if (node.hasResource($enum.NODE_RESOURCE.IMAGE_SOURCE)) {
                        ...
                        let widthPercent = node.has('width', $enum.CSS_STANDARD.PERCENT) ? width : 0;
                        let heightPercent = node.has('height', $enum.CSS_STANDARD.PERCENT) ? height : 0;
                        let scaleType = 'fitXY';
                        let imageSet: ImageSrcSet[] | undefined;
                        if (element.srcset) {
                            imageSet = $css.getSrcSet(element, this.localSettings.supported.imageFormat);
                            if (imageSet.length && imageSet[0].actualWidth) {
            ...
            if (node.imageElement) {
                ...
                if (image) {
                    ...
                    if (maxWidth > 0) {
                        ...
                        else if (image.width >= parent.box.width && node.css('maxWidth') === '100%') {
                    ...
                    if (maxHeight > 0) {
                        ...
                        else if (image.height >= parent.box.height && node.css('maxHeight') === '100%') {
