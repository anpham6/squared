            ...
            const boxWidth = (() => {
                ...
                if (renderParent) {
                    if (renderParent.overflowX) {
                        ...
                        $util.captureMap(node.documentParent.actualChildren, item => item.float === 'left' && item.siblingIndex < node.siblingIndex, item => floatStart = Math.max(floatStart, item.linear.right));
