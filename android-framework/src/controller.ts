...
function setLeftTopAxis(node: View, parent: View, hasDimension: boolean, horizontal: boolean) {
    const [orientation, dimension, posA, posB, marginA, marginB, paddingA, paddingB] = horizontal ? [STRING_ANDROID.HORIZONTAL, 'width', 'left', 'right', BOX_STANDARD.MARGIN_LEFT, BOX_STANDARD.MARGIN_RIGHT, BOX_STANDARD.PADDING_LEFT, BOX_STANDARD.PADDING_RIGHT]
    ...
    if (hasDimension && autoMargin[orientation]) {
        ...
        if (node.hasPX(posA)) {
            ...
            if (node.centerAligned) {
                node.anchorParent(STRING_ANDROID.HORIZONTAL, 0.5);
            }
            else {
...
function setVerticalAlignment(node: View) {
    ...
    if (!isNaN(bias)) {
        node.anchorStyle(STRING_ANDROID.VERTICAL, bias, 'packed', false);
    }
}
...
const getRelativeVertical = (layout: squared.base.LayoutUI<View>) => layout.some(item => item.positionRelative || !item.pageFlow && item.autoPosition) ? CONTAINER_NODE.RELATIVE : CONTAINER_NODE.LINEAR;
...
export default class Controller<T extends View> extends squared.base.ControllerUI<T> implements android.base.Controller<T> {
    ...
    public processUnknownParent(layout: squared.base.LayoutUI<T>) {
        ...
        if (node.has('columnCount') || node.hasPX('columnWidth')) {
            ...
            layout.setContainerType(CONTAINER_NODE.CONSTRAINT, layout.floated.size ? NODE_ALIGNMENT.FLOAT : NODE_ALIGNMENT.SEGMENTED);
        }
    ...
    public processTraverseVertical(layout: squared.base.LayoutUI<T>) {
        ...
        if (layout.some((item, index) => item.lineBreakTrailing && index < layout.length - 1)) {
            ...
            else if (cleared.size || this.checkFrameHorizontal(layout)) {
                layout.addRender(NODE_ALIGNMENT.FLOAT);
                layout.addRender(NODE_ALIGNMENT.HORIZONTAL);
            }
            else {
        ...
        switch (layout.floated.size) {
            case 1:
                ...
                return (<View> layout.item(0)).floating && (
                    layout.linearY ||
                    layout.length > 2 && !layout.singleRowAligned ||
    ...
    public checkLinearHorizontal(layout: squared.base.LayoutUI<T>) {
        ...
        if ((floated.size === 0 || floated.size === 1 && floated.has('left')) && layout.node.lineHeight === 0 && layout.singleRowAligned) {
            ...
            for (const node of layout) {
                if (!(node.naturalChild && node.length === 0 && !node.inputElement && !node.controlElement && !node.positionRelative && node.css('verticalAlign') === 'baseline' && !node.blockVertical && node.zIndex === 0 && (lineHeight === 0 || node.lineHeight === lineHeight && node.fontSize === fontSize))) {
                    return false;
                }
            }
            return true;
            ...
            if (node.layoutRelative) {
                ...
                for (const item of renderChildren) {
                    if (!item.positioned) {
                        if (item.pageFlow || item.autoPosition) {
                            ...
                            if (!horizontal) {
                                item.anchorParent(STRING_ANDROID.HORIZONTAL, 0);
                            }
                            if (!vertical) {
                ...
                if (j > 0) {
                    ...
                    if (node.layoutHorizontal && !node.layoutElement) {
                        ...
                        const { horizontal, vertical } = item.constraint;
                        if (!horizontal) {
                            item.anchorParent(STRING_ANDROID.HORIZONTAL, item.centerAligned ? 0.5 : (item.rightAligned ? 1 : 0), 'packed', false);
                        }
                        if (!vertical) {
    ...
    protected processConstraintHorizontal(node: T, children: T[]) {
        ...
        for (let i = 0; i < length; i++) {
            ...
            if (previous) {
                ...
                item.anchorStyle(STRING_ANDROID.HORIZONTAL, bias);
            }
            if (item.pageFlow) {
                if (item !== baseline) {
                    if (item.controlElement) {
                        ...
                        switch (item.css('verticalAlign')) {
                            ...
                            case 'middle':
                                if (baseline?.textElement === false || textBottom) {
                                    ...
                                    item.anchorParent(STRING_ANDROID.VERTICAL, 0.5);
                                }
                                break;
                            case 'text-bottom':
                                if (textBaseline === null) {
        ...
        if (baseline) {
            if (tallest && tallest !== baseline && baseline.textElement && getMaxHeight(tallest) > getMaxHeight(baseline)) {
                switch (tallest.verticalAlign) {
                    case 'middle':
                        baseline.anchorParent(STRING_ANDROID.VERTICAL, 0.5, 'packed', true);
                        break;
                    case 'baseline':
                ...
                if (valid && baseline.baselineElement && !baseline.imageOrSvgElement && node.ascend({ condition: (item: T) => item.layoutHorizontal, error: (item: T) => item.layoutVertical || item.layoutGrid, attr: 'renderParent' }).length) {
                    ...
                    baseline.anchorParent(STRING_ANDROID.VERTICAL, 0);
        ...
        for (const item of children) {
            if (item.css('columnSpan') === 'all') {
                ...
                if (item.multiline) {
                    ...
                    maxSize = maxSize > 0 ? Math.min(width, maxSize) : width;
                }
        ...
        for (let i = 0; i < length; i++) {
            ...
            if (row.length === 1) {
                ...
                if (i === 0) {
                    ...
                    rowStart.anchorStyle(STRING_ANDROID.VERTICAL, 0);
                }
                else if (previousRow) {
                ...
                rowStart.anchorParent(STRING_ANDROID.HORIZONTAL, rowStart.rightAligned ? 1 : (rowStart.centerAligned ? 0.5 : 0));
            }
            else {
                const columns: T[][] = [];
            ...
            const applyLayout = (seg: T[], reverse: boolean) => {
                ...
                if (!floating && parent.css('textAlign') === 'center') {
                    ...
                    if (reverse) {
                        ...
                        rowStart.anchorStyle(STRING_ANDROID.HORIZONTAL, 0);
                    }
                }
                ...
                for (let j = 0; j < lengthA; j++) {
                    ...
                    if (i === 0) {
                        if (length === 1) {
                            ...
                            chain.anchorStyle(STRING_ANDROID.VERTICAL, 0);
                        }
                    }
                    else if (currentRowTop === undefined && i === length - 1) {
            ...
            if (i > 0) {
                if (previousRow.length === 1) {
                    ...
                    if (aboveRowEnd && currentRowTop) {
                        aboveRowEnd.anchor('bottomTop', barrierId);
                    }
                    else {
                        for (const chain of previousRow) {
                            chain.anchor('bottomTop', barrierId);
                        }
                    }
                    for (const chain of partition) {
                        setVerticalAlignment(chain);
